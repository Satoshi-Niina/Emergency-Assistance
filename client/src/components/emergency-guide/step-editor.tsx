...
const StepEditor: React.FC<StepEditorProps> = ({
    ...
    try {
      ...
      if (stepToUpdate && stepToUpdate.images) {
        ...
        if (existingImage) {
          ...
          // 譌｢蟄倥・逕ｻ蜒上ｒ蜑企勁
          const updatedImages = stepToUpdate.images.filter(
      ...
      console.log('名・・逕ｻ蜒上い繝・・繝ｭ繝ｼ繝蔚RL:', uploadUrl);
      ...
      if (currentStepToUpdate) {
        ...
        if (currentImages.length < 3) {
          ...
          console.log('名・・逕ｻ蜒上ｒ蜊ｳ蠎ｧ縺ｫUI縺ｫ蜿肴丐:', {
            stepId,
          ...
          // 逕ｻ蜒剰ｿｽ蜉蠕後・閾ｪ蜍穂ｿ晏ｭ假ｼ医ヵ繧｡繧､繝ｫ荳隕ｧ縺ｫ謌ｻ繧峨↑縺・ｼ・
          if (onSave) {
            setTimeout(() => {
              onSave();
            }, 100);
      ...
      alert(
        `逕ｻ蜒上い繝・・繝ｭ繝ｼ繝峨↓螟ｱ謨励＠縺ｾ縺励◆: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    } finally {
      setUploadingImages(prev => ({ ...prev, [stepId]: false }));
    }
      ...
      if (imageIndex >= 0 && imageIndex < newImages.length) {
        ...
        if (confirmDelete) {
          try {
            ...
            console.log('卵・・逕ｻ蜒丞炎髯､URL:', deleteUrl);
            ...
            if (!response.ok) {
              ...
              throw new Error(
                errorData.error ||
                  '繧ｵ繝ｼ繝舌・荳翫・逕ｻ蜒上ヵ繧｡繧､繝ｫ蜑企勁縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・
              );
            }
            ...
            alert(
              `逕ｻ蜒丞炎髯､縺ｫ螟ｱ謨励＠縺ｾ縺励◆: ${error instanceof Error ? error.message : '荳肴・縺ｪ繧ｨ繝ｩ繝ｼ'}`
            );
          }
        }
      }
  ...
  const handleDrop = (stepId: string, e: React.DragEvent) => {
    ...
    if (imageFile) {
      console.log('名・・繝峨Λ繝・げ&繝峨Ο繝・・縺ｧ逕ｻ蜒上い繝・・繝ｭ繝ｼ繝・', {
        stepId,
    ...
    return (
      <div className='space-y-4' style={{ minHeight: '0px' }}>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          <div>
            <Label htmlFor={`title-${step.id}`} className='text-base-2x'>
              繧ｿ繧､繝医Ν
            </Label>
            <Input
              id={`title-${step.id}`}
              value={step.title}
              onChange={e =>
                handleStepFieldChange(step.id, 'title', e.target.value)
              }
          ...
          <div>
            <Label htmlFor={`description-${step.id}`} className='text-base-2x'>
              隱ｬ譏・
            </Label>
            <Input
              ...
              onChange={e =>
                handleStepFieldChange(step.id, 'description', e.target.value)
              }
        ...
        <div>
          <Label htmlFor={`message-${step.id}`} className='text-base-2x'>
            繝｡繝・そ繝ｼ繧ｸ
          </Label>
          <Textarea
            id={`message-${step.id}`}
            value={step.message}
            onChange={e =>
              handleStepFieldChange(step.id, 'message', e.target.value)
            }
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4'>
              ...
              {(step.images ?? []).filter(image => image && image.url && image.url.trim() !== '').map((image, index) => (
                <div key={index} className='relative group aspect-video'>
                  {(() => {
                    // 繝励Ξ繝薙Η繝ｼ縺ｨ蜷後§逕ｻ蜒酋RL螟画鋤繧剃ｽｿ逕ｨ - buildImageUrl髢｢謨ｰ繧堤ｵｱ荳菴ｿ逕ｨ
                    const convertedUrl = buildImageUrl(image.url);
                    
                    console.log('名・・StepEditor逕ｻ蜒上Ξ繝ｳ繝繝ｪ繝ｳ繧ｰ:', {
                      stepId: step.id,
                      imageIndex: index,
                    ...
                    return (
                      <img
                        ...
                        onError={e => {
                          console.error('笶・StepEditor逕ｻ蜒剰ｪｭ縺ｿ霎ｼ縺ｿ繧ｨ繝ｩ繝ｼ:', {
                            originalUrl: image.url,
                            convertedUrl: convertedUrl,
                            fileName: image.fileName,
                            stepId: step.id,
                          ...
                          // 逕ｻ蜒剰ｪｭ縺ｿ霎ｼ縺ｿ繧ｨ繝ｩ繝ｼ譎ゅ・髱櫁｡ｨ遉ｺ縺ｫ縺吶ｋ
                          e.currentTarget.style.display = 'none';
                        }}
                        onLoad={() => {
                          console.log('笨・StepEditor逕ｻ蜒剰ｪｭ縺ｿ霎ｼ縺ｿ謌仙粥:', {
                            originalUrl: image.url,
                            convertedUrl: convertedUrl,
                            fileName: image.fileName,
                            stepId: step.id,
                ...
                >
                  <div className='text-center'>
                    ...
                    <p className='text-xs text-gray-500'>
                      {(step.images ?? []).filter(image => image && image.url && image.url.trim() !== '').length} / 3譫・
                    </p>
            ...
            {step.images && step.images.length > 0 && (
              <div className='mt-4 p-4 bg-blue-50 border border-blue-200 rounded text-base-2x text-blue-700'>
                ...
                <ul className='mt-2 space-y-2'>
                  <li>
                    窶｢ 蜷後§繝輔ぃ繧､繝ｫ蜷阪・逕ｻ蜒上・閾ｪ蜍慕噪縺ｫ驥崎､・→縺励※讀懷・縺輔ｌ縺ｾ縺・
                  </li>
                  <li>
                    窶｢ 驥崎､・判蜒上・譌｢蟄倥・繝輔ぃ繧､繝ｫ繧貞・蛻ｩ逕ｨ縺励※繧ｹ繝医Ξ繝ｼ繧ｸ繧堤ｯ邏・＠縺ｾ縺・
                  </li>
  ...
  const ActionButtons = () => (
    <div className='flex justify-between items-center p-4 bg-gray-50 rounded-lg border'>
      <div className='text-sm text-gray-500'>繧ｹ繝ｩ繧､繝画焚: {steps.length}</div>
    ...
    <div className='h-full flex flex-col'>
      ...
      <div className='p-4 bg-yellow-50 border border-yellow-200 rounded mb-4 flex-shrink-0'>
        ...
        <p className='text-yellow-700 text-sm'>
          蜿励￠蜿悶▲縺殱teps.length: {steps.length}
        </p>
        <p className='text-yellow-700 text-sm'>
          steps蜀・ｮｹ: {steps.map(s => `${s.id}:${s.title}`).join(', ')}
        </p>
        <p className='text-yellow-700 text-sm'>
          expandedSteps: {Object.keys(expandedSteps).length}蛟・
        </p>
      </div>
                          ...
                          >
                            ...
                            >
                              <div
                                ...
                                className='absolute top-1/2 -left-8 -translate-y-1/2 p-2 cursor-grab text-gray-400 hover:text-gray-600'
                              >
                                <GripVertical />
                              </div>
                              <CardContent
      ...
      {onSave && onCancel && (
        <div className='flex justify-end gap-3 pt-4 border-t bg-white mt-4 flex-shrink-0'>
          ...
          <Button onClick={onSave}>菫晏ｭ・/Button>
        </div>
      )}
    </div>
  );

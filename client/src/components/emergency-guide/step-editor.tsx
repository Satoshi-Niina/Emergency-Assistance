...
import { convertImageUrl, buildImageUrl } from '../../lib/image-utils';

/**
 * ⚠️ AI編集禁止 このファイルはスライド編集UI専用です
 * - タイトル編集機能の変更は禁止
 * - 条件分岐UI構造の変更は禁止
 * - バックエンド連携コードの追加は禁止
 */
...
interface Step {
  ...
  // 古いプププロパティは後方互換性のために残す（封印されたものは削除済み）
  imageUrl?: string;
  imageFileName?: string;
}

interface StepditorProps {
  steps: Step[];
...
const Stepditor: React.FC<StepditorProps> = ({
  steps,
  onStepUpdate,
  onStepsReorder,
  onStepDelete,
  ...
  console.log('🔄 Stepditor レンダリング開始', {
      ...
      if (stepToUpdate && stepToUpdate.images) {
        const existingImage = (stepToUpdate.images ?? []).find(
          img =>
            ...
            img.fileName === file.name.replace(/\.[^/.]+$/, '') // 拡張子を除いて比較
        );
  ...
  const handleDrop = (stepId: string, e: React.Dragvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (imageFile) {
      console.log('🖼️ ドラッグ&ドロップで画像アップロード処理', {
        stepId,
  ...
  const handleDragnd = (result: DropResult) => {
              ...
              {(step.images ?? []).filter(image => image && image.url && image.url.trim() !== '').map((image, index) => (
                <div key={index} className='relative group aspect-video'>
                  {(() => {
                    ...
                    console.log('🖼️ Stepditor画像レンダリング:', {
                      stepId: step.id,
                      imageIndex: index,
            ...
            {step.images && step.images.length > 0 && (
              <div className='mt-4 p-4 bg-blue-50 border border-blue-200 rounded text-base-2x text-blue-700'>
                ...
                <ul className='mt-2 space-y-2'>
                  <li>
                    • 同じファイル名の画像は自動的に重複して検出されます
                  </li>
                  <li>
                    • 重複画像は既存のファイルを再利用してストレージを節約します
                  </li>
  ...
  const ActionButtons = () => (
    <div className='flex justify-between items-center p-4 bg-gray-50 rounded-lg border'>
      <div className='text-sm text-gray-500'>スライド数: {steps.length}</div>
  ...
  return (
    <div className='h-full flex flex-col'>
      ...
      <div className='p-4 bg-yellow-50 border border-yellow-200 rounded mb-4 flex-shrink-0'>
        <p className='text-yellow-800 font-medium'>Stepditor デバッグ情報:</p>
        <p className='text-yellow-700 text-sm'>
          受け取ったsteps.length: {steps.length}
        </p>
        <p className='text-yellow-700 text-sm'>
          steps配列: {steps.map(s => `${s.id}:${s.title}`).join(', ')}
        </p>
        <p className='text-yellow-700 text-sm'>
          expandedSteps: {Object.keys(expandedSteps).length}個
        </p>
      </div>
      ...
      >
        <DragDropContext onDragnd={handleDragnd}>

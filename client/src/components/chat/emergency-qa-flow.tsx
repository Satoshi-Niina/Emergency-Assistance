...
const ENGINE_START_FLOW: QAFlowStep[] = [
  ...
  },
  {
    ...
    question:
      'バッテリーは接続されていますか？（もしくはメインブレーカーが入っていますか？）',
    type: 'choice',
...
export default function EmergencyQAFlow({
  ...
  const getNextStep = useCallback(
    (currentStepId: string, answer: string): QAFlowStep | null => {
      ...
      for (const condition of currentStep.nextStepCondition) {
        if (answer.includes(condition.condition)) {
          return (
            flowSteps.find(step => step.id === condition.nextStepId) || null
          );
        }
      }
              ...
              <div className='flex items-start space-x-3'>
                <div className='flex-shrink-0 w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center'>
                  {(() => {
                    ...
                    if (
                      questionText.includes('時間') ||
                      questionText.includes('緊急')
                    ) {
                      return <Clock className='h-4 w-4 text-red-600' />;
                    } else if (
                      questionText.includes('照明') ||
                      questionText.includes('点灯')
                    ) {
                      return <Zap className='h-4 w-4 text-yellow-600' />;
                    } else if (
                      questionText.includes('スターター') ||
                      questionText.includes('キー')
                    ) {

...
const KnowledgeUploader: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    if (!validExtensions.includes(fileExt)) {
      toast({
        title: '未対応のファイル形式',
        description:
          'PDF, Word, Excel, PowerPoint, またはテキストファイルのみアップロード可能です',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/knowledge/upload`,
        {
          method: 'POST',
          body: formData,
        }
  ...
  const fetchDocuments = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/knowledge`
      );
      if (!response.ok) {
    ...
    <div className='space-y-8'>
      <Card className='bg-white border border-cyan-200 shadow-sm mb-6'>
        ...
        <CardContent className='pt-6'>
          <div className='mb-6'>
            <Label htmlFor='file-upload' className='text-cyan-700 font-medium'>
              ファイルを選択してください
            </Label>
            ...
            <p className='text-sm text-cyan-600 mt-2'>
              対応フォーマット: PDF, Word (.docx), Excel (.xlsx), PowerPoint
              (.pptx), テキストファイル (.txt)
            </p>
          </div>
        </CardContent>
      ...
      <Card className='bg-white border border-cyan-200 shadow-sm'>
        ...
        <CardContent className='pt-6'>
          {isLoading ? (
            ...
            <Table>
              <TableCaption className='text-cyan-500'>
                AI回答生成に使用されるナレッジ文書一覧
              </TableCaption>
              <TableHeader>
                ...
                {documents.map(doc => (
                  <TableRow key={doc.id} className='hover:bg-cyan-50'>
                    ...
                    <TableCell className='text-cyan-700'>
                      {new Date(doc.addedAt).toLocaleString('ja-JP')}
                    </TableCell>
                    <TableCell>
                      <div className='flex space-x-1'>
                        <Button
                          ...
                          size='icon'
                          onClick={() =>
                            handleProcessDocument(doc.id, doc.title)
                          }

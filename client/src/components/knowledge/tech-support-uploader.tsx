...
const TechSupportUploader: React.FC = () => {
    ...
    if (!isValidFileFormat(selectedFile, processingType)) {
      if (processingType === 'document') {
        toast({
          title: '未対応のファイル形式',
          description:
            'PDF, Word, Excel, PowerPoint ファイルのみアップロード可能です',
          variant: 'destructive',
        });
      } else {
        toast({
          title: '未対応の画像形式',
          description:
            'SVG, PNG, JPG, GIF 画像ファイルのみアップロード可能です',
          variant: 'destructive',
        });
      }
      return;
  ...
  const handleDetectDuplicateImages = async () => {
    try {
      ...
      const response = await fetch(
        '/api/tech-support/detect-duplicate-images',
        {
          method: 'POST',
        }
      );
          ...
          <div className='flex flex-wrap gap-3'>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    一時ファイルをクリーンアップしてストレージスペースを解放します
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    同じ内容の重複画像を検出して削除します（タイムスタンプが異なる同じ画像を削除）
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    アプリケーションのキャッシュをクリアして最新のデータを読み込みます
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>

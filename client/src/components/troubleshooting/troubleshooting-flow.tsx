  ...
  return (
    <Card className="w-full max-w-md mx-auto">
      ...
      <CardFooter className="flex flex-col gap-3">
            ...
            if (currentStep && flowData) {
              try {
                ...
                displayedSteps.forEach((step, index) => {
                  if (step) {
                    guideContent += `${index + 1}. ${step.title || step.id}\n`;
                    const stepText = getStepText(step);
                    if (stepText) {
                      guideContent += `   ${stepText.replace(/\n/g, '\n   ')}\n\n`;
                    }
                  }
                });

                // 履歴の概要を追加
                const stepTitles = displayedSteps.map(step => step?.title || step?.id).filter(Boolean);
                guideContent += `【フロー概要】\n${stepTitles.join(' → ')}\n\n`;
                ...
                const guideData = {
                  ...
                  steps: displayedSteps.map(step => ({
                    id: step?.id,
                    title: step?.title,
                    description: getStepText(step)
                  }))
                };

                // 緊急ガイドとして送信
                console.log('緊急ガイドを送信中:', guideData);
                await sendEmergencyGuide(guideData);
                ...
                toast({
                  title: '送信エラー',
                  description: '応急処置ガイドの送信に失敗しました。再度お試しください。',
                  variant: 'destructive',
                });
              }
            }
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-message-square mr-2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2z"/></svg>
          表示した手順をチャットに送信
        </Button>
      </CardFooter>
    </Card>

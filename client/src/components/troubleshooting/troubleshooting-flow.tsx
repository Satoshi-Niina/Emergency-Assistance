...
export default function TroubleshootingFlow({
  ...
  const [flowData, setFlowData] = useState<TroubleshootingFlowData | null>(
    null
  );
  ...
  const sendToChat = useCallback(() => {
    ...
    window.dispatchEvent(
      new CustomEvent('emergency-guide-completed', { detail: chatData })
    );
  ...
  const handleNext = (nextStepId?: string) => {
    ...
    if (nextStepId) {
      ...
      const currentIndex = flowData.steps.findIndex(
        ...
        <div className='space-y-6'>
          ...
          <div className='flex justify-between items-center pt-4 border-t'>
            <Button
              ...
              disabled={
                flowData.steps.findIndex(s => s.id === currentStep.id) === 0
              }
            >

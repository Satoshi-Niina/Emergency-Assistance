...
export default function ChatPage() {
  ...
  // 繧､繝ｳ繧ｿ繝ｩ繧ｯ繝・ぅ繝冶ｨｺ譁ｭ繝｢繝ｼ繝峨・迥ｶ諷狗ｮ｡逅・
  ...
  const fetchMachineTypes = useCallback(async () => {
    try {
      ...
      const response = await fetch(apiUrl, {
        ...
        credentials: 'include' // 繧ｻ繝・す繝ｧ繝ｳ邯ｭ謖√・縺溘ａ
      });
  ...
  // AI謾ｯ謠ｴ髢句ｧ具ｼ医う繝ｳ繧ｿ繝ｩ繧ｯ繝・ぅ繝冶ｨｺ譁ｭ繝｢繝ｼ繝峨↓螟画峩・・
  const handleStartAiSupport = async () => {
    try {
      // 繧､繝ｳ繧ｿ繝ｩ繧ｯ繝・ぅ繝冶ｨｺ譁ｭ繝｢繝ｼ繝峨ｒ髢句ｧ・
      setInteractiveDiagnosisMode(true);
      
      toast({
        title: "AI謾ｯ謠ｴ髢句ｧ・,
        description: "繧､繝ｳ繧ｿ繝ｩ繧ｯ繝・ぅ繝匁腐髫懆ｨｺ譁ｭ繧帝幕蟋九＠縺ｾ縺・,
      });
    } catch (error) {
  ...
  // AI謾ｯ謠ｴ邨ゆｺ・ｼ医う繝ｳ繧ｿ繝ｩ繧ｯ繝・ぅ繝冶ｨｺ譁ｭ繝｢繝ｼ繝臥ｵゆｺ・ｼ・
  const handleAiSupportExit = () => {
    setInteractiveDiagnosisMode(false);
    
    toast({
      title: "AI謾ｯ謠ｴ邨ゆｺ・,
      description: "繧､繝ｳ繧ｿ繝ｩ繧ｯ繝・ぅ繝匁腐髫懆ｨｺ譁ｭ繧堤ｵゆｺ・＠縺ｾ縺励◆",
    });
  };
    ...
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      {interactiveDiagnosisMode ? (
        ...
        <>
          ...
          {/* 繝｡繝・そ繝ｼ繧ｸ蜈･蜉帙お繝ｪ繧｢・磯壼ｸｸ繝√Ε繝・ヨ繝｢繝ｼ繝会ｼ・*/}

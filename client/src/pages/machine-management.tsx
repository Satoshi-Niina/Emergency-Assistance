...
export default function MachineManagementPage() {
  ...
  const handleMachineSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: editingMachine ? "機械番号を更新しました" : "機械番号を追加しました",
  ...
  const handleTypeDelete = async (typeId: string, typeName: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/machines/machine-types/${typeId}`, {
        method: 'DELETE'
      });
  ...
  // 機械番号削除
  const handleMachineDelete = async (machineId: string, machineNumber: string) => {
    if (!confirm(`機械番号「${machineNumber}」を削除してもよろしいですか？`)) return;

    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/machines/machines/${machineId}`, {
        method: 'DELETE'
      });
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-400">機種と機械番号の一覧表示、追加、編集、削除ができます</p>
          </div>
          <Link to="/settings">
                ...
                <div className="divide-y">
                  {machineTypes.map((type) => (
                    <div key={type.id} className="p-4 hover:bg-gray-50">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-medium text-gray-900">{type.machine_type_name}</p>
                          <p className="text-sm text-gray-500">
                            {machines.filter(m => m.machine_type_id === type.id).length}台の機械
                          </p>
                        </div>
                        <div className="flex gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleTypeDelete(type.id, type.machine_type_name)}
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleMachineDelete(machine.id, machine.machine_number)}
                              className="h-8 w-8 p-0 text-red-600 hover:text-red-700 hover:bg-red-50"
                            >
                              <Trash2 className="h-3 w-3" />
                            </Button>

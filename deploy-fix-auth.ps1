# Azure App Service 認証修正デプロイスクリプト
# セーフモードを有効にして認証エラーを解決

Write-Host "🚀 Azure App Service 認証修正デプロイを開始..." -ForegroundColor Green
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        Write-Host "✅ /me エンドポイント成功: $($response.StatusCode)" -ForegroundColor Green
        Write-Host "📄 レスポンス: $($response.Content)" -ForegroundColor Gray
    } else {
        Write-Host "⚠️ /me エンドポイント警告: $($response.StatusCode)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "❌ /me エンドポイント失敗: $($_.Exception.Message)" -ForegroundColor Red
}

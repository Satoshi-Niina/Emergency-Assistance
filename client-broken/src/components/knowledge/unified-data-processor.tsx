...
const UnifiedDataProcessor: React.FC = () => {
  ...
  const handleProcessFile = async () => {
    ...
    if (!validExtensions.includes(fileExt)) {
      toast({
        title: '未対応のファイル形式',
        description:
          'PDF, Word, Excel, PowerPoint, またはテキストファイルのみ処理可能です',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      formData.append(
        ...
        options.extractKnowledgeBase.toString()
      );
      formData.append(
        'extractImageSearch',
        options.extractImageSearch.toString()
      );
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const processedDocuments = documentsData.map(
      ...
      <div className='bg-white p-6 rounded-lg shadow-md border border-blue-100'>
        ...
        <div className='mb-4'>
          <p className='text-base font-semibold text-gray-700'>
            PPTX, PDF,
            DOCX、PDFをアップロードすると、システムは自動的に内容を解析し、検索とAIの応答に適した形式に変換します。この処理には数分かかることがあります。
          </p>
        </div>

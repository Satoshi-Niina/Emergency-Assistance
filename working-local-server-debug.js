...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'ログイン処理に失敗しました' });
  }
});
    ...
    const knowledgeData = [
      {
        id: 'kb-1',
        title: 'エンジンオイル漏れの対処法',
        content: 'エンジンオイルが漏れている場合は、まず漏れ箇所を特定し、応急処置としてオイルを補充してください。',
      ...
      },
      {
        id: 'kb-2',
        title: 'ブレーキ異音の原因と対処',
        content: 'ブレーキの異音は主にパッドの摩耗が原因です。点検を行い、必要に応じて交換してください。',
        category: 'maintenance',
        tags: ['ブレーキ', '異音', 'パッド'],
        createdAt: new Date().toISOString()
      }
...
app.get('/api/troubleshooting/:id', async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(404).json({ success: false, message: 'トラブルシューティングデータが見つかりません' });
    }
    ...
    res.status(500).json({ success: false, message: 'トラブルシューティング詳細の取得に失敗しました' });
  }
});
...
app.get('/api/flows', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: '応急処置フローの取得に失敗しました' });
  }
});
...
app.get('/api/flows/:id', async (req, res) => {
  try {
    ...
    if (!flow) {
      return res.status(404).json({ success: false, message: 'フローが見つかりません' });
    }
    ...
    res.status(500).json({ success: false, message: '個別フローの取得に失敗しました' });
  }
});

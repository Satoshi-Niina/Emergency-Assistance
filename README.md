...
# 自動更新機能付きで起動
.\docker-dev.ps1 start    # PowerShell
./docker-dev.sh start     # Bash
...
### 🔄 自動更新の仕組み
- **ファイル監視**: TypeScript/JavaScriptファイルの変更を自動検知
- **自動同期**: コード変更を即座にコンテナに反映
- **自動再ビルド**: package.jsonやDockerfile変更時に自動再ビルド
- **ホットリロード**: サーバーが自動で再起動
...
# 開発環境の操作
.\docker-dev.ps1 start    # 起動（自動更新有効）
.\docker-dev.ps1 stop     # 停止
.\docker-dev.ps1 restart  # 再起動
.\docker-dev.ps1 logs     # ログ表示
.\docker-dev.ps1 status   # 状態確認
.\docker-dev.ps1 clean    # クリーンアップ
```

### 📁 監視対象ファイル
- **自動同期**: `*.ts`, `*.js`, `*.json`
- **自動再ビルド**: `package.json`, `Dockerfile`, `*.config.*`
- **除外**: `node_modules/`, `dist/`, `logs/`, `uploads/`, `temp/`

  ...
  try {
    ...
    if (searchType === 'all' || searchType === 'db') {
      const dbResults = await Promise.all([
        ...
        .where(
          or(
            like(supportHistory.machineType, `%${query}%`),
            like(supportHistory.machineNumber, `%${query}%`),
            sql`${supportHistory.jsonData}::text ILIKE ${'%' + query + '%'}`
          )
        )
        .limit(limit)
        .offset(offset),
        ...
        .where(
          or(
            like(historyItems.title, `%${query}%`),
            like(historyItems.description, `%${query}%`)
          )
        )
        .limit(limit)
        .offset(offset)
...
// ディレクトリ内のファイル検索関数
async function searchInDirectory(dirPath: string, query: string): Promise<Array<{
  type: string;
  name: string;
  path: string;
  size: number;

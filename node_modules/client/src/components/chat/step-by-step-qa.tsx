...
export default function StepByStepQA({
  ...
  const generateSolution = async (allAnswers: QAAnswer[]) => {
    try {
      const solution = await qaManager.generateSolution(
        allAnswers,
        knowledgeBase
      );
      setProgress(100);
        ...
        <CardHeader className='pb-3'>
          <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center space-x-2'>
              ...
              <Button variant='outline' size='sm' onClick={resetQA}>
                ...
                <div className='flex-shrink-0 w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center'>
                  {(() => {
                    ...
                    if (
                      ...
                      questionText.includes('危険')
                    ) {
                      return <Shield className='h-4 w-4 text-red-600' />;
                    } else if (
                      questionText.includes('症状') ||
                      questionText.includes('異常')
                    ) {
                      return (
                        <AlertCircle className='h-4 w-4 text-orange-600' />
                      );
                    } else if (
                      questionText.includes('原因') ||
                      questionText.includes('なぜ')
                    ) {
                      return <Search className='h-4 w-4 text-blue-600' />;
                    } else if (
                      questionText.includes('対応') ||
                      questionText.includes('処置')
                    ) {
                      return <Wrench className='h-4 w-4 text-green-600' />;
                    } else if (
                      questionText.includes('確認') ||
                      questionText.includes('チェック')
                    ) {

  ...
  useEffect(() => {
    ...
    const handlePreviewImage = (e: Event) => {
      ...
      if (customEvent.detail) {
        ...
        // 全スライドの配列を設定
        if (
          customEvent.detail.all_slides &&
          Array.isArray(customEvent.detail.all_slides)
        ) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <div className='sr-only'>
          ...
          <p id='image-preview-description'>
            画像のプレビュー表示と関連情報の確認ができます
          </p>
        </div>
        <div className='w-full flex justify-between items-center p-2 bg-blue-700 text-white'>
          <h3 className='text-sm font-medium ml-2'>
            ...
            {allSlides.length > 1 &&
              ` - スライド ${currentSlideIndex + 1}/${allSlides.length}`}
          </h3>
            ...
            onError={e => {
              ...
              try {
                ...
                if (allSlides.length > 1) {
                  ...
                  console.log(
                    `現在のスライドが表示できないため、次のスライド(${nextIndex})に切り替え`
                  );
                  changeSlide('next');
                  return;
                }
                ...
                if (parent) {
                  ...
                  if (!parent.querySelector('.error-message')) {
                    ...
                    errorDiv.className =
                      'error-message absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-50 text-white p-4 rounded-lg z-20';
                ...
                {content ? (
                  ...
                  <div className='text-sm'>
                    <h4 className='font-medium mb-1'>
                      {currentSlideInfo.タイトル ||
                        `スライド ${currentSlideInfo.スライド番号}`}
                    </h4>

                    {currentSlideInfo.本文 &&
                      currentSlideInfo.本文.length > 0 && (
                        <div className='mb-2'>
                  ...
                  <img
                    ...
                    onLoad={e => {
                      ...
                      if (parent) {
                        // 読み込み中プレースホルダーを非表示に
                        const placeholders =
                          parent.querySelectorAll('.animate-spin');
                        placeholders.forEach(ph => {
                          if (ph.parentElement) {
                            ph.parentElement.style.display = 'none';
                          }
                    ...
                    onError={e => {
                      ...
                      if (parent) {
                        ...
                        errorOverlay.className =
                          'absolute inset-0 flex items-center justify-center z-20 bg-black bg-opacity-50 text-white text-xs';

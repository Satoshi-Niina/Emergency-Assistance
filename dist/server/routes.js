...
import { addDocumentToKnowledgeBase, listKnowledgeBaseDocuments, removeDocumentFromKnowledgeBase } from './lib/knowledge-base';
...
export async function registerRoutes(app) {
    ...
    app.post('/api/knowledge/upload', requireAuth, requireAdmin, upload.single('file'), async (req, res) => {
        try {
            ...
            try {
                const docId = await addDocumentToKnowledgeBase({ originalname: path.basename(filePath), path: filePath, mimetype: 'text/plain' }, fs.readFileSync(filePath, 'utf-8'));
    ...
    app.post('/api/knowledge/:docId/process', requireAuth, requireAdmin, async (req, res) => {
        try {
            ...
            if (documents.success && documents.documents) {
                ...
                // 再処理を実行
                const newDocId = await addDocumentToKnowledgeBase({ originalname: path.basename(docPath), path: docPath, mimetype: 'text/plain' }, fs.readFileSync(docPath, 'utf-8'));
                res.json({
                    success: true,
                    docId: newDocId,

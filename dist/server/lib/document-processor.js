 ...
 * @returns Extracted text
 */
export async function extractExcelText(filePath) {
    try {
...
export async function extractPptxText(filePath) {
    try {
        ...
        try {
            ...
            // 埋め込み画像を探す（メディアフォルダ内）
            const mediaEntries = zipEntries.filter((entry) => entry.entryName.startsWith('ppt/media/') &&
      ...
      `;
        ...
        const newVehicleData = {
            ...
            all_slides: allSlidesUrls.length > 0 ? allSlidesUrls :
                Array.from({ length: 4 }, (_, i) => `/knowledge-base/images/${slideImageBaseName}_${(i + 1).toString().padStart(3, '0')}.png`),
 ...
 */
export async function extractTxtText(filePath) {
    try {
 ...
 */
export async function storeDocumentChunks(document) {
 ...
 */
export async function findRelevantChunks(query) {

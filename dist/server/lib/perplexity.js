 ...
 * @returns 生成された回答
 */
export async function processPerplexityRequest(query, systemPrompt = "", useKnowledgeBaseOnly = true) {
    // API キーをチェック
    if (!validateApiKey()) {
    ...
    try {
        ...
        const response = await axios.post('https://api.perplexity.ai/chat/completions', requestOptions, {
            headers: {

    ...
    app.post('/api/data-processor/process', upload.single('file'), async (req, res) => {
        try {
            ...
            if (extractKnowledgeBase) {
                // ナレッジベースに追加
                const result = await addDocumentToKnowledgeBase({ originalname: path.basename(filePath), path: filePath, mimetype: 'text/plain' }, fs.readFileSync(filePath, 'utf-8'));
            ...
            if (createQA) {
                try {
                    ...
                    const generateQAPairs = openaiModule.generateQAPairs;
                    // QAペアの初期化
                    let qaPairs = [];
                    // 既に処理されたドキュメントを使用
                    if (processedDocument) {

...
async function checkTakabeni2() {
  try {
    ...
    // takabeni2ユーザーを検索
    const user = await db.select().from(users).where(eq(users.username, 'takabeni2')).limit(1);
    ...
    // bcryptテスト
    try {
      const bcryptMatch = await bcrypt.compare(testPassword, takabeni2.password);
    ...
    console.log(`- bcrypt形式: ${takabeni2.password.startsWith('$2b$') ? 'はい' : 'いいえ'}`);

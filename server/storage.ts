...
export const storage = {
  ...
  getMessagesForChat: async (chatId: string) =>
    dbStorage.getMessagesForChat(chatId),
  getMessagesForChatAfterTimestamp: async (chatId: string, timestamp: number) =>
    dbStorage.getMessagesForChatAfterTimestamp(chatId, timestamp),
  createMessage: async (message: any) => dbStorage.createMessage(message),
  clearChatMessages: async (chatId: string) =>
    dbStorage.clearChatMessages(chatId),
  // Media methods
  getMedia: async (id: string) => dbStorage.getMedia(id),
  getMediaForMessage: async (messageId: string) =>
    dbStorage.getMediaForMessage(messageId),
  createMedia: async (media: any) => dbStorage.createMedia(media),
  // Keyword methods
  getKeywordsForDocument: async (documentId: string) =>
    dbStorage.getKeywordsForDocument(documentId),
  createKeyword: async (keyword: any) => dbStorage.createKeyword(keyword),
  searchDocumentsByKeyword: async (keyword: string) =>
    dbStorage.searchDocumentsByKeyword(keyword),
  // Chat export methods
  saveChatExport: async (chatId: string, userId: string, timestamp: number) =>
    dbStorage.saveChatExport(chatId, userId, timestamp),
  getLastChatExport: async (chatId: string) =>
    dbStorage.getLastChatExport(chatId),
};

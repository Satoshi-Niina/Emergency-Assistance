...
function authenticateToken(req, res, next) {
  ...
  if (!process.env.JWT_SECRET) {
    console.error('[security] JWT_SECRET is not set. Set JWT_SECRET in your environment for authentication to work.');
    return res.status(500).json({ success: false, error: 'jwt_secret_not_configured', message: 'JWT_SECRET is not configured on the server' });
  }
  ...
  try {
    ...
    if (dbPool) {
      try {
        ...
        // JWTトークンを生成
        if (!process.env.JWT_SECRET) {
          console.error('[security] JWT_SECRET is not set - cannot sign tokens');
          return res.status(500).json({ success: false, error: 'jwt_secret_not_configured', message: 'JWT_SECRET is not configured on the server' });
        }
    ...
    function handleSimpleAuth(username, password, res) {
      ...
      console.log(`Provided credentials: username="${username}", password="${password}"`);
        ...
        // Development fallback: read test credentials from env variables.
        // To enable, set BYPASS_DB_FOR_LOGIN=true AND SEED_ADMIN_PASSWORD / SEED_NIINA_PASSWORD as needed.
        const bypass = process.env.BYPASS_DB_FOR_LOGIN === 'true';
        const envAdminPwd = process.env.SEED_ADMIN_PASSWORD || null;
        const envNiinaPwd = process.env.SEED_NIINA_PASSWORD || null;

        if (!bypass) {
          console.log('Development fallback disabled (BYPASS_DB_FOR_LOGIN not true)');
          return res.status(401).json({ success: false, error: 'invalid_credentials', message: 'ユーザー名またはパスワードが正しくありません' });
        }

...
router.get('/list', async (_req, res) => {
  try {
    console.log(
      '🔍 トラブルシューティングディレクトリからフロー一覧を取得中（/list）...'
    );
...
// フロー詳細取得エンドポイント
router.get('/detail/:id', async (_req, res) => {
  try {
  ...
  try {
    ...
    if (!fs.existsSync(filePath)) {
      return res.status(404).json({
        error: 'ファイルが存在しません',
        fileName,
        emergencyFlowsPath: path.join(
          __dirname,
          '../../knowledge-base/images/emergency-flows',
          fileName
        ),
        chatExportsPath: path.join(
          __dirname,
          '../../knowledge-base/images/chat-exports',
          fileName
        ),
        emergencyFlowsDir: fs.existsSync(

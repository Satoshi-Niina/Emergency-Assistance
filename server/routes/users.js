    ...
    try {
        ...
        if (!existingUser) {
            ...
            if (!existingUser) {
                ...
                // 部分一致を試行
                const partialMatches = allUsers.filter(u => u.id.includes(id) ||
                    id.includes(u.id) ||
                ...
                if (partialMatches.length > 0) {
                    ...
                    console.log(`[ERROR] 文字コード比較:`, allUsers.map(u => ({
                        id: u.id,
                        charCodes: Array.from(u.id).map(c => c.charCodeAt(0)),
                        searchId: id,
                        searchCharCodes: Array.from(id).map(c => c.charCodeAt(0)),
                    })));
                    return res.status(404).json({
                        error: 'User not found',
                        searchId: id,
                        availableIds: allUsers.map(u => u.id),
                        possibleMatches: allUsers.filter(u => u.id.includes(id) ||
                            id.includes(u.id) ||
                            u.id.toLowerCase() === id.toLowerCase()),
                    });
                }
...
router.post('/import-excel', requireAuth, requireAdmin, upload.single('file'), async (req, res) => {
    try {

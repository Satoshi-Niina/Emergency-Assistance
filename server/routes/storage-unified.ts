...
const norm = (p: string) =>
  [BASE, String(p || '')]
    ...
    .join('/')
    .replace(/\\+/g, '/')
    .replace(/\/+/g, '/'); // 連続スラッシュも統一

/** Extract title from JSON content */
const extractTitle = async (containerClient: any, blobName: string): Promise<string> => {
  try {
...
router.get('/list', async (req, res) => {
  try {
    ...
    const items: Array<{ name: string; updatedAt: string; sizeBytes: number; title: string; etag: string }> = [];
    ...
    console.log(`✅ Storage list completed: ${items.length} items`);
    res.json(items);
  } catch (error) {
...
router.get('/json/:name', async (req, res) => {
  try {
    ...
    console.log('📄 JSON get request:', { originalName: name, normalizedName });
    ...
    const content = await azureStorage.readFileAsString(normalizedName);
...
router.put('/json/:name', async (req, res) => {
  try {
    ...
    await azureStorage.writeStringToFile(normalizedName, content);
    ...
    console.error('❌ JSON put error:', error);
    res.status(500).json({
...
router.get('/image-url', async (req, res) => {
  try {
    ...
    const normalizedName = norm(name);
    console.log('🖼️ Image SAS request:', { 
      originalName: name, 
      normalizedName 
    });

...
import { validateInput, logSecurityEvent, strictLimiter } from '../middleware/security';
...
// セキュアなユーザー登録
router.post('/register', strictLimiter, validateInput(registerValidation), async (req, res) => {
  try {
    ...
    if (existingUser.length > 0) {
      logSecurityEvent('REGISTRATION_FAILED_USERNAME_EXISTS', { username }, req);
...
router.post('/change-password', strictLimiter, validateInput(changePasswordValidation), async (req, res) => {
  try {
    ...
    if (!isCurrentPasswordValid) {
      logSecurityEvent('PASSWORD_CHANGE_FAILED_INVALID_CURRENT', { userId }, req);

...
export function registerChatRoutes(app: any): void {
  ...
  app.get('/api/chats', requireAuth, async (req, res) => {
    ...
    const chats = await storage.getChatsForUser(
      String((req as any).session.userId ?? '')
    );
  ...
  app.post('/api/chats/:id/messages/system', requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `システムメッセージ送信: chatId=${chat.id}, chatUserId=${chat.userId}, sessionUserId=${(req as any).session.userId}`
      );
  ...
  app.post('/api/chats/:id/messages', requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `チャットアクセス: chatId=${chat.id}, chatUserId=${chat.userId}, sessionUserId=${(req as any).session.userId}`
      );
      ...
      const [aiMessage] = await (db as any)
        .insert(messages)
        .values({
          chatId: chatId,
          senderId: 'ai',
  ...
  app.post('/api/chats/:id/clear', requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `チャット履歴クリア開始: chatId=${chatId}, force=${force}, clearAll=${clearAll}`
      );
      const chat = await storage.getChat(chatId);
      if (!chat) {
        return res.status(404).json({ message: 'Chat not found' });
      ...
      console.log(
        `チャット履歴クリア: chatId=${chat.id}, chatUserId=${chat.userId}, sessionUserId=${(req as any).session.userId}`
      );
      ...
      try {
        ...
        for (const message of beforeMessages) {
          try {
            ...
            console.error(
              `メディア削除エラー (messageId: ${message.id}):`,
              mediaError
            );
          }
        }
        ...
        console.log(
          `削除後のメッセージ数: ${afterCount}, 削除されたメッセージ数: ${deletedMessageCount}`
        );
          ...
          if (force || clearAll) {
            ...
            for (const remainingMessage of afterMessages) {
              try {
                ...
                console.error(
                  `個別削除エラー (messageId: ${remainingMessage.id}):`,
      ...
      console.log(
        `チャット履歴クリア完了: chatId=${chatId}, 削除メッセージ数=${deletedMessageCount}, 削除メディア数=${deletedMediaCount}, 最終メッセージ数=${finalCount}`
      );
  ...
  app.post('/api/chats/:id/export', requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `チャット ${chatId} のエクスポート処理完了（ファイルベース）`
      );
  ...
  app.post('/api/chats/:id/send-test', async (req, res) => {
    try {
      ...
      const imagesDir = process.env.FAULT_HISTORY_IMAGES_DIR || path.join(
        projectRoot,
        'knowledge-base',
        'images',
        'chat-exports'
      ...
      const userMessages = cleanedChatData.messages.filter(
        (m: any) => !m.isAiResponse
      );
      ...
      if (isProduction && shouldUseAzure && azureStorageService) {
        // 本番環境: Azure BLOBに直接アップロード
        const tempPath = path.join(require('os').tmpdir(), fileName);
        try {
          fs.writeFileSync(tempPath, jsonString, 'utf8');
          const blobName = `${azureJsonPrefix}${fileName}`;
          await azureStorageService.uploadFile(tempPath, blobName);
          fs.unlinkSync(tempPath);
          console.log('✅ JSONをBLOBにアップロード:', blobName);
          console.log('保存されたデータサイズ:', Buffer.byteLength(jsonString, 'utf8'), 'bytes');
        } catch (uploadError) {
          if (fs.existsSync(tempPath)) fs.unlinkSync(tempPath);
          console.error('⚠️ BLOBアップロードエラー:', uploadError);
          throw uploadError;
        }
      } else {
        // 開発環境: ローカルファイルに保存
  ...
  app.post('/api/chats/:id/send', requireAuth, async (req, res) => {
    try {
      ...
      const projectRoot = path.resolve(__dirname, '..', '..');
      const resolveRelativePath = (targetPath: string): string =>
        path.isAbsolute(targetPath)
          ? targetPath
          : path.join(projectRoot, targetPath);

      const exportsDir = resolveRelativePath(
      ...
      const resolveImageLink = async (
        imageFileName: string,
        absolutePath: string
      ): Promise<{
        url: string;
        storageKey: string;
        storageType: 'azure-blob' | 'local-file';
      }> => {
        ...
        if (shouldUseAzure && fs.existsSync(absolutePath)) {
          ...
          try {
            ...
            try {
              url = azureStorageService.generateBlobSasUrl(
                blobName,
                60 * 60 * 1000
              );
              storageKey = blobName;
      ...
      console.log('🔍 元のメッセージ内容:', cleanedChatData.messages.map((m: any) => ({
        id: m.id,
        isAiResponse: m.isAiResponse,
      ...
      for (const message of cleanedChatData.messages) {
        ...
        if (
          message.content &&
          message.content.includes('/api/images/chat-exports/')
        ) {
          try {
            // URLからファイル名を抽出
            const urlMatch = message.content.match(/\/api\/images\/chat-exports\/([^"'\s]+)/);
            if (urlMatch && urlMatch[1]) {
              const imageFileName = urlMatch[1];
              const imagePath = path.join(imagesDir, imageFileName);
              const { url: resolvedUrl, storageKey, storageType } =
              ...
              savedImages.push({
                ...
                storageType,
                mimeType: 'image/jpeg',
                fileSize: fs.existsSync(imagePath)
                  ? fs.statSync(imagePath).size.toString()
                  : '0',
                description: `Chat image ${imageFileName}`,
                createdAt: new Date().toISOString(),
              });

              message.content = resolvedUrl;
      ...
      console.log('🔍 処理後のメッセージ内容:', cleanedChatData.messages.map((m: any) => ({
        id: m.id,
        isAiResponse: m.isAiResponse,
        contentPreview: m.content?.substring(0, 100)
      })));
      ...
      let exportStorage: {
        type: 'azure-blob' | 'local-file';
        key: string;
      } = { type: 'local-file', key: filePath };
  ...
  app.get('/api/chats/:id/export-formatted', requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `フォーマット済みエクスポート: chatId=${chat.id}, chatUserId=${chat.userId}, sessionUserId=${userId}`
      );
      if (

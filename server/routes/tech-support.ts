...
const extractPptxText = async (
  filePath: string
): Promise<{ text: string; slideImages: string[] }> => {
  try {
    ...
    // 知識ベース画像ディレクトリを確保
    const knowledgeBaseImagesDir = path.join(
      process.cwd(),
    ...
    console.log(
      `✅ PPTXファイル処理完了: ${extractedText.length}文字を抽出、${slideImages.length}枚の画像を生成`
    );
    return {
    ...
    throw new Error(
      `PPTXファイルの処理に失敗しました: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
};
...
async function detectAndRemoveDuplicateImages() {
  const knowledgeImagesDir: any = path.join(
    process.cwd(),
    'knowledge-base/images'
  );
  let removedCount = 0;
  ...
  try {
    ...
    console.log(
      `knowledge-base/imagesディレクトリ内の画像ファイル数: ${imageFiles.length}件`
    );
    ...
    for (const entry of Array.from(prefixGroups.entries()) as [
      string,
      string[],
    ][]) {
      ...
      if (files.length > 1) {
        console.log(
          `プレフィックス "${prefix}" で ${files.length}件の潜在的な重複ファイルを検出`
        );
    ...
    for (const entry of Array.from(fileHashes.entries()) as [
      string,
      string[],
    ][]) {
      ...
      if (filePaths.length > 1) {
        console.log(
          `ハッシュ値 ${hash} で ${filePaths.length}件の重複ファイルを検出`
        );
        ...
        const latestFileIndex: any = timestamps.indexOf(
          Math.max(...timestamps)
        );
        // 最新以外のファイルを削除
...
async function cleanupRedundantFiles() {
  ...
  try {
    ...
    console.log(
      `知識ベースディレクトリ内のファイル数: ${knowledgeImages.length}件`
    );
    // 各アップロードディレクトリをチェック
    for (const dir of uploadsDirs) {
      ...
      console.log(
        `ディレクトリ内のファイル数: ${dir} - ${uploadedFiles.length}件`
      );
    ...
    console.log(
      `重複ファイル削除結果: 成功=${removedCount}件, 失敗=${errorCount}件`
    );
    return { removed: removedCount, errors: errorCount };
  } catch (error) {
...
async function verifyAndCleanupFile(filePath: any, subDir) {
  try {
    ...
    if (fs.existsSync(kbTargetPath)) {
      ...
      fs.unlinkSync(filePath);
      console.log(
        `uploads内のファイルを削除しました (knowledge-baseに存在確認済み): ${filePath}`
      );
    } else {
      console.log(
        `警告: knowledge-baseに対応するファイルが見つからないため、削除をスキップします: ${filePath}`
      );
    }
  } catch (error) {
    console.error(
      `ファイルの検証・クリーンアップに失敗しました: ${filePath}`,
      error
    );
  }
}
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        const relPath: any = path.relative(
    ...
    console.error(
      `ディレクトリの検証・クリーンアップに失敗しました: ${dirPath}`,
      error
    );
  }
}
...
const storage: any = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    const originalName: any = Buffer.from(file.originalname, 'latin1').toString(
      'utf8'
    );
    // ファイル名に使用できない文字を除去し、スペースをアンダースコアに変換
    const sanitizedName: any = originalName
      .split('.')[0]
      .replace(/[\/\\:*?"<>|]/g, '')
      .replace(/\s+/g, '_');
...
const upload: any = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedExtensions.includes(ext)) {
      ...
      cb(
        new Error(
          `サポートされていないファイル形式です。サポート形式: ${allowedExtensions.join(', ')}`
        )
      );
    }
  },
...
router.post('/clear-cache', async (_req, res) => {
  try {
    ...
    const indexJsonPath: any = path.join(
      process.cwd(),
    ...
    try {
      ...
      // 有効なメタデータファイルのみを追加
      const validFiles: any = jsonFiles.filter(
        file =>
          file.endsWith('_metadata.json') && !blacklistFiles.includes(file)
      );
      console.log('有効なJSONファイル:', validFiles);
      // インデックスに追加
      for (const file of validFiles) {
        try {
          const content: any = fs.readFileSync(
  ...
  try {
    ...
    for (const jsonDir of jsonDirs) {
      if (fs.existsSync(jsonDir)) {
        ...
        console.log(
          `${jsonDir}内の有効なメタデータファイル: ${files.length}件`
        );
...
router.post('/init-image-search-data', async (_req, res) => {
  try {
    ...
    const existingDataPath: any = path.join(
      knowledgeBaseDataDir,
    ...
    if (fs.existsSync(existingDataPath)) {
      try {
        ...
        console.log(
          `既存の画像検索データを読み込みました: ${existingData.length}件（実在ファイルのみ）`
        );
      } catch (error) {
              ...
              if (
                ...
                if (imageText && imageText['画像パス']) {
                  ...
                  if (existingImageFiles.includes(imagePath)) {
                    // 詳細な説明文を生成
                    const slideTitle: any =
                      slide['タイトル'] || `スライド ${index + 1}`;
                    const slideContent: any = slide['本文']
                      ? slide['本文'].join('。')
                      : '';
    ...
    newData.forEach(newItem => {
      const exists: any = combinedData.some(
    ...
    const validData: any = combinedData.filter(
      item => item.file && existingImageFiles.includes(item.file)
    );
    // データをファイルに保存
    fs.writeFileSync(
    ...
    console.log(
      `データを統合しました: ${validData.length}件（新規: ${newCount}件）`
    );
...
router.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    console.log(
      `元ファイル保存: ${keepOriginalFile ? '有効' : '無効（デフォルト）'}`
    );
    ...
    if (
      ...
      try {
        ...
        const knowledgeBaseDataDir: any = path.join(
          __dirname,
          '../../knowledge-base/data'
        );
        ...
        const imageSearchDataPath: any = path.join(
          knowledgeBaseDataDir,
          'image_search_data.json'
        );
        // 画像検索データの初期化
        ...
        if (fs.existsSync(imageSearchDataPath)) {
          try {
            const jsonContent: any = fs.readFileSync(
              imageSearchDataPath,
              'utf8'
            );
            imageSearchData = JSON.parse(jsonContent);
            console.log(
              `既存の画像検索データを読み込みました: ${imageSearchData.length}件`
            );
          } catch (jsonErr) {
        ...
        const newImageItem = {
          ...
          metadata: {
            ...
            originalFileType:
              fileExt !== '.png' ? fileExt.substring(1).toUpperCase() : 'PNG',
            sourcePath: updatedFilePath || filePath,
            originalPath:
              originalFilePath !== updatedFilePath ? originalFilePath : '',
            documentId: fileId.split('_')[0], // ドキュメントIDの関連付け
          },
        };
        // 既存のデータに新しいアイテムを追加または更新
        const existingIndex: any = imageSearchData.findIndex(
          item => item.id === fileId
        );
        if (existingIndex >= 0) {
        ...
        fs.writeFileSync(
          imageSearchDataPath,
          JSON.stringify(imageSearchData, null, 2)
        );
        console.log(
          `画像検索データを知識ベースに更新しました: ${imageSearchData.length}件`
        );
    ...
    try {
      ...
      const extractedDataPath: any = path.join(
        knowledgeBaseDataDir,
        'extracted_data.json'
      );
      ...
      // 既存データの読み込み
      const extractedData: any = JSON.parse(
        fs.readFileSync(extractedDataPath, 'utf-8')
      );
      // 車両データキーが存在するか確認
      ...
      const newData = {
        ...
        image_path:
          (metadata as any).type === 'pptx'
            ? (metadata as any).slideImages[0]
            : null,
        all_slides:
          (metadata as any).type === 'pptx'
            ? (metadata as any).slideImages
            : null,
      ...
      // 車両データに追加
      const existingIndex: any = vehicleData.findIndex(
...
router.post('/sync-directories', async (_req, res) => {
  try {
    ...
    const rootDir: any = path.join(__dirname, '../../');
    const knowledgeBaseImagesDir: any = path.join(
      rootDir,
      'knowledge-base/images'
    );
    const tempImageDirs = [
      ...
      for (const file of files) {
        ...
        if (!fs.existsSync(targetPath)) {
          try {
            ...
            console.log(
              `ファイルをknowledge-baseにコピーしました: ${sourcePath} -> ${targetPath}`
            );
      ...
      for (const targetDir of tempImageDirs) {
        ...
        if (!fs.existsSync(targetPath)) {
          try {
            ...
            fs.copyFileSync(sourcePath, targetPath);
            console.log(
              `ファイルを一時ディレクトリにコピーしました: ${sourcePath} -> ${targetPath}`
            );
...
async function cleanupOrphanedJsonFiles() {
  ...
  try {
    ...
    // メタデータJSONファイル一覧を取得
    const allFiles: any = fs.readdirSync(jsonDir);
    const metadataFiles: any = allFiles.filter(
      file => file.endsWith('_metadata.json') && !blacklistFiles.includes(file)
    );
    console.log(
      `JSONディレクトリ内のメタデータファイル: ${metadataFiles.length}件`
    );
    // knowledge-base内のドキュメントディレクトリ一覧を取得
    const knowledgeBaseDir: any = path.join(process.cwd(), 'knowledge-base');
    const docDirs: any = fs
    ...
    console.log(
      `知識ベース内のドキュメントプレフィックス: ${docDirs.length}件`
    );
    // 各メタデータファイルをチェック
    for (const file of metadataFiles) {
      ...
      const hasMatchingDocument: any = docDirs.some(
        docPrefix => docPrefix === prefix
      );
    ...
    console.log(
      `孤立したJSONファイル削除結果: 成功=${removedCount}件, 失敗=${errorCount}件`
    );
    return { removed: removedCount, errors: errorCount };
  } catch (error) {
    console.error(
      '孤立したJSONファイルのクリーンアップ中にエラーが発生しました:',
      error
    );
    return { removed: removedCount, errors: errorCount + 1 };
  }

...
router.get('/all-machines', async (_req, res) => {
  try {
    ...
    console.log(
      `✅ 全機械データ取得完了: ${Object.keys(groupedData).length}機種`
    );

    // 本番環境用ログ出力
    console.log({
      route: '/api/machines/all-machines',
...
// 機種追加API
router.post('/machine-types', async (_req, res) => {
  try {
...
// 機械番号追加API
router.post('/machines', async (_req, res) => {
  try {
...
// 機種更新API
router.put('/machine-types/:id', async (_req, res) => {
  try {
    const { id } = req.params;
    const { machine_type_name } = req.body;
    ...
    console.log('✅ 機種更新完了:', updatedMachineType[0]);

    res.json({
      success: true,
      data: updatedMachineType[0],
...
// 機種削除API
router.delete('/machine-types/:id', async (_req, res) => {
  try {
    const { id } = req.params;
...
// 機械番号更新API
router.put('/machines/:id', async (_req, res) => {
  try {
    const { id } = req.params;
    const { machine_number, machine_type_id } = req.body;
    ...
    console.log('✅ 機械番号更新完了:', updatedMachine[0]);
...
// 機械番号削除API
router.delete('/machines/:id', async (_req, res) => {
  try {
    const { id } = req.params;

...
10. 最終ステップでは必ず具体的な対応結果や次のステップを明示し、利用者が次にとるべき行動を明確にしてください。`;
        ...
        const generatedFlow = await processOpenAIRequest(prompt);
        // OpenAI APIエラーの確認
        if (typeof generatedFlow === 'string' &&
...
router.get('/detail/:id', (_req, res) => {
    try {
        const cleanFlowId = req.params.id.startsWith('ts_')
        ...
        const decisionStepsDetail = decisionSteps.map(step => ({
            id: step.id,
            title: step.title,
        ...
        const conditionStepsDetail = conditionSteps.map(step => ({
            id: step.id,
            title: step.title,
            description: step.description,
            message: step.message,

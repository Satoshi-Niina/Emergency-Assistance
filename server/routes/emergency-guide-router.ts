...
const cleanupSpecificFiles = () => {
    try {
        ...
        const problemFile: any = path.join(kbJsonDir, 'guide_1744876404679_metadata.json');
        if (fs.existsSync(problemFile)) {
            console.log('蝠城｡後→縺ｪ繧九ヵ繧｡繧､繝ｫ繧貞炎髯､縺励∪縺・', problemFile);
            fs.unlinkSync(problemFile);
        }
        // 髢｢騾｣縺吶ｋ逕ｻ蜒上ｒ蜑企勁
        if (fs.existsSync(kbImageDir)) {
            const imageFiles: any = fs.readdirSync(kbImageDir);
            const relatedImages: any = imageFiles.filter(img => img.startsWith('guide_1744876404679'));
            relatedImages.forEach(imgFile => {
                const imgPath: any = path.join(kbImageDir, imgFile);
                if (fs.existsSync(imgPath)) {
                    fs.unlinkSync(imgPath);
                    console.log('髢｢騾｣逕ｻ蜒上ｒ蜑企勁縺励∪縺励◆:', imgPath);
                }
            });
        }
    }
...
async function processJsonFile(filePath) {
    try {
        ...
        console.log(`蜈・・繝輔ぃ繧､繝ｫ繝代せ: ${filePath}`);
        console.log(`蜈・・繝輔ぃ繧､繝ｫ蜷・ ${path.basename(filePath)}`);
...
router.post('/process', upload.single('file'), async (req, res) => {
    try {
        ...
        if (fileExtension === '.json') {
            log(`JSON繝輔ぃ繧､繝ｫ蜃ｦ逅・ ${filePath}`);
            result = await processJsonFile(filePath);
        }
        else if (['.pptx', '.ppt'].includes(fileExtension)) {
            log(`PowerPoint繝輔ぃ繧､繝ｫ蜃ｦ逅・ ${filePath}`);
            result = await processPowerPointFile(filePath);
        }
        else {
            return res.status(400).json({
                success: false,
                error: '繧ｵ繝昴・繝医＆繧後※縺・↑縺・ヵ繧｡繧､繝ｫ蠖｢蠑上〒縺吶ら樟蝨ｨ縺ｮ蜃ｦ逅・・PowerPoint縺ｨJSON縺ｮ縺ｿ繧ｵ繝昴・繝医＠縺ｦ縺・∪縺吶・
            });
        }
        ...
        if (autoGenerateFlow) {
            ...
            try {
                console.log(`閾ｪ蜍輔ヵ繝ｭ繝ｼ逕滓・繧帝幕蟋・ ${result.id}`);
                ...
                fetch(`http://localhost:${process.env.PORT || 3000}/api/flow-generator/generate-from-guide/${result.id}`, {
                    ...
                    if (response.ok) {
                        ...
                        console.log(`繝輔Ο繝ｼ逕滓・謌仙粥: ${generationResult.flowData.id}`);
                    }
                    else {
...
router.get('/list', (_req, res) => {
    try {
        ...
        console.log('- JSON繝・ぅ繝ｬ繧ｯ繝医Μ縺ｮ蜀・ｮｹ:', fs.readdirSync(kbJsonDir));
...
router.get('/detail/:id', (req, res) => {
    try {
        ...
        const files: any = fs.readdirSync(kbJsonDir)
                ...
                if (slide.逕ｻ蜒上ユ繧ｭ繧ｹ繝・&& Array.isArray(slide.逕ｻ蜒上ユ繧ｭ繧ｹ繝・) {
                    slide.逕ｻ蜒上ユ繧ｭ繧ｹ繝・forEach((imgText) => {
                        if (imgText.逕ｻ蜒上ヱ繧ｹ && imgText.逕ｻ蜒上ヱ繧ｹ.startsWith('/uploads/')) {
                            ...
                            console.log(`逕ｻ蜒上ヱ繧ｹ繧呈峩譁ｰ: ${imgText.逕ｻ蜒上ヱ繧ｹ}`);
                        }
                    });
                }
            });
...
router.delete('/delete/:id', async (req, res) => {
    try {
        ...
        console.log(`繝槭ャ繝√☆繧九ヵ繧｡繧､繝ｫ (${matchingFiles.length}莉ｶ):`, matchingFiles);
        if (matchingFiles.length === 0) {
            return res.status(404).json({ error: `謖・ｮ壹＆繧後◆繧ｬ繧､繝・(ID: ${id}) 縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ` });
        }
        ...
        try {
            ...
            console.warn(`蜑企勁蜑阪・繝輔ぃ繧､繝ｫ蜀・ｮｹ隱ｭ縺ｿ蜿悶ｊ縺ｫ螟ｱ謨・ ${mainFilePath}`, readError);
        }
        ...
        for (const file of matchingFiles) {
            ...
            if (fs.existsSync(filePath)) {
                ...
                console.log(`JSON繝輔ぃ繧､繝ｫ繧貞炎髯､縺励∪縺励◆: ${filePath}`);
                deletedCount++;
            }
        }
        console.log(`蜑企勁縺輔ｌ縺櫟SON繝輔ぃ繧､繝ｫ謨ｰ: ${deletedCount}莉ｶ`);
            ...
            try {
                ...
                if (Array.isArray(indexData.guides)) {
                    ...
                    if (beforeCount !== afterCount) {
                        ...
                        console.log(`繧､繝ｳ繝・ャ繧ｯ繧ｹ縺九ｉ蜑企勁縺励∪縺励◆: ${beforeCount - afterCount}繧ｨ繝ｳ繝医Μ`);
                    }
                }
            }
            catch (indexError) {
        ...
        console.log(`蠢懈･蜃ｦ鄂ｮ繧ｬ繧､繝峨ｒ蜑企勁縺励∪縺励◆: ID=${id}, 繧ｿ繧､繝医Ν=${title}`);
        // 蜑企勁蠕後・譛邨ら｢ｺ隱搾ｼ医ヵ繧｡繧､繝ｫ繧ｷ繧ｹ繝・Β繧貞・繝√ぉ繝・け・・
        const remainingFiles: any = fs.readdirSync(kbJsonDir);
        console.log('----------- 蜑企勁蠕後・迥ｶ諷・-----------');
        console.log('蜑企勁縺励◆ID:', id);
        console.log('蜑企勁蠕後・繝・ぅ繝ｬ繧ｯ繝医Μ蜀・ｮｹ:', remainingFiles);
        console.log('蜑企勁縺励◆繝輔ぃ繧､繝ｫ:', matchingFiles);
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            for (const file of matchingFiles) {
                ...
                if (fs.existsSync(filePath)) {
                    allDeleted = false;
                    console.log(`蜑企勁縺御ｸ榊ｮ悟・縺ｪ縺溘ａ蠑ｷ蛻ｶ蜀崎ｩｦ陦・(${attempt + 1}/3): ${filePath}`);
                    try {
                        // 繝輔ぃ繧､繝ｫ繧貞ｼｷ蛻ｶ逧・↓蜑企勁
                        fs.unlinkSync(filePath);
                        console.log(`  竊・蜑企勁謌仙粥: ${filePath}`);
                    }
                    catch (e) {
                        ...
                        try {
                            if (fs.existsSync(filePath)) {
                                fs.unlinkSync(filePath);
                                console.log(`  竊・2蝗樒岼縺ｮ蜑企勁縺梧・蜉・ ${filePath}`);
                            }
                        }
                        catch (e2) {
            ...
            if (allDeleted) {
                console.log(`縺吶∋縺ｦ縺ｮ繝輔ぃ繧､繝ｫ縺梧ｭ｣蟶ｸ縺ｫ蜑企勁縺輔ｌ縺ｾ縺励◆ (隧ｦ陦・ ${attempt + 1}蝗樒岼縺ｧ螳御ｺ・`);
                break;
            }
            // 谺｡縺ｮ隧ｦ陦悟燕縺ｫ蟆代＠蠕・ｩ・
        ...
        setTimeout(() => {
            try {
                for (const file of matchingFiles) {
                    ...
                    if (fs.existsSync(filePath)) {
                        fs.unlinkSync(filePath);
                        console.log(`繝舌ャ繧ｯ繧ｰ繝ｩ繧ｦ繝ｳ繝牙炎髯､: ${filePath}`);
                    }
                }
                ...
                if (fs.existsSync(troubleshootingDir)) {
                    ...
                    for (const tsFile of tsFiles) {
                        if (tsFile.includes(id.split('_')[1])) {
                            ...
                            console.log(`繝舌ャ繧ｯ繧ｰ繝ｩ繧ｦ繝ｳ繝牙炎髯､・医ヨ繝ｩ繝悶Ν繧ｷ繝･繝ｼ繝・ぅ繝ｳ繧ｰ・・ ${tsFilePath}`);
                        }
                    }
                }
            }
...
router.post('/send', async (req, res) => {
    try {
        ...
        // 繝ｭ繧ｰ蜃ｺ蜉帛ｼｷ蛹・
        console.log('------------------------------------');
        console.log('蠢懈･蜃ｦ鄂ｮ繧ｬ繧､繝峨ョ繝ｼ繧ｿ繧偵メ繝｣繝・ヨ縺ｫ騾∽ｿ｡:');
        console.log(`chatId: ${chatId}`);
        console.log(`title: ${guideData.title || "辟｡鬘・}`);
        ...
        try {
            ...
            console.log('繝√Ε繝・ヨ繝｡繝・そ繝ｼ繧ｸ繧剃ｽ懈・縺励∪縺励◆:', userMessage.id, aiMessage.id);
            return res.json({
                success: true,
                userMessage,
                aiMessage
            ...
            console.error('繝｡繝・そ繝ｼ繧ｸ菴懈・荳ｭ縺ｫ繝・・繧ｿ繝吶・繧ｹ繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆:', dbError);
            return res.status(500).json({
                success: false,
                message: "繝｡繝・そ繝ｼ繧ｸ縺ｮ菫晏ｭ倅ｸｭ縺ｫ繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆",
                error: dbError instanceof Error ? dbError.message : "繝・・繧ｿ繝吶・繧ｹ繧ｨ繝ｩ繝ｼ"
            });
        }
    }
    catch (error) {
...
router.post('/system-message', async (req, res) => {
    try {
        ...
        // 繝ｭ繧ｰ蜃ｺ蜉・
        console.log('------------------------------------');
        console.log('繧ｷ繧ｹ繝・Β繝｡繝・そ繝ｼ繧ｸ繧偵メ繝｣繝・ヨ縺ｫ騾∽ｿ｡:');
        console.log(`chatId: ${chatId}`);
        ...
        try {
            ...
            console.log('繧ｷ繧ｹ繝・Β繝｡繝・そ繝ｼ繧ｸ繧剃ｽ懈・縺励∪縺励◆:', message.id);
            return res.json({
                success: true,
                message
            });
            ...
            if (appStorage) {
                ...
                console.log('莉｣譖ｿ繧ｹ繝医Ξ繝ｼ繧ｸ縺ｧ繧ｷ繧ｹ繝・Β繝｡繝・そ繝ｼ繧ｸ繧剃ｽ懈・縺励∪縺励◆:', message.id);
                return res.json({
                    success: true,
                    message
                });
...
router.post('/send-to-chat/:guideId/:chatId', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: '蠢懈･蜃ｦ鄂ｮ繧ｬ繧､繝峨・繝√Ε繝・ヨ縺ｸ縺ｮ騾∽ｿ｡縺ｫ螟ｱ謨励＠縺ｾ縺励◆' });
    }
});
...
router.post('/save-chat-local', async (req, res) => {
    try {
            ...
            if (message.content && typeof message.content === 'string') {
                ...
                while ((match = base64ImageRegex.exec(message.content)) !== null) {
                    ...
                    try {
                        ...
                        console.log(`逕ｻ蜒丈ｿ晏ｭ伜ｮ御ｺ・ ${imageFileName}`);
                    } catch (imageError) {
                    ...
                    if (mediaItem.type === 'image' && mediaItem.url && mediaItem.url.startsWith('data:image/')) {
                        ...
                        if (base64Match) {
                            ...
                            try {
                                ...
                                console.log(`繝｡繝・ぅ繧｢逕ｻ蜒丈ｿ晏ｭ伜ｮ御ｺ・ ${imageFileName}`);
                            } catch (imageError) {
        ...
        console.log(`繝√Ε繝・ヨ菫晏ｭ伜ｮ御ｺ・ ${filePath}`);
        console.log(`菫晏ｭ倥＆繧後◆繝｡繝・そ繝ｼ繧ｸ謨ｰ: ${processedMessages.length}`);
        console.log(`菫晏ｭ倥＆繧後◆逕ｻ蜒乗焚: ${chatData.metadata.imageCount}`);

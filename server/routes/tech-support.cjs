...
exports.techSupportRouter = void 0;
...
const extractPptxText = async (filePath) => {
    try {
        ...
        const extractDir = path_1.default.join(tempDir, `pptx_${Date.now()}`);
        // 一時ディレクトリを作成
        ...
        // スライドXMLファイルを探す
        const slidesDir = path_1.default.join(extractDir, 'ppt', 'slides');
        const slideFiles = fs_1.default.existsSync(slidesDir)
            ? fs_1.default
        ...
        const slideImages = [];
        // 知識ベース画像ディレクトリを確保
        const knowledgeBaseImagesDir = path_1.default.join(process.cwd(), 'knowledge-base/images');
        if (!fs_1.default.existsSync(knowledgeBaseImagesDir)) {
            fs_1.default.mkdirSync(knowledgeBaseImagesDir, { recursive: true });
        }
        // 各スライドのテキストを抽出
        for (let i = 0; i < slideFiles.length; i++) {
            const slideNumber = i + 1;
            const slideFilePath = path_1.default.join(slidesDir, slideFiles[i]);
            const slideContent = fs_1.default.readFileSync(slideFilePath, 'utf8');
            // テキスト内容の抽出
            ...
            // ノート（スピーカーノート）の内容を取得
            const noteFilePath = path_1.default.join(extractDir, 'ppt', 'notesSlides', `notesSlide${slideNumber}.xml`);
            if (fs_1.default.existsSync(noteFilePath)) {
                const noteXml = fs_1.default.readFileSync(noteFilePath, 'utf8');
            ...
            const imagePath = path_1.default.join(knowledgeBaseImagesDir, imageFileName);
            // プレースホルダー画像を生成
            try {
                const svgContent = `
                ...
                `;
                await (0, sharp_1.default)(Buffer.from(svgContent)).png().toFile(imagePath);
...
const ensureDirectoryExists = (dirPath) => {
    ...
    try {
        // 絶対パスの場合は相対パスに変換
        const relativePath = path_1.default.isAbsolute(dirPath)
            ? path_1.default.join(process.cwd(), path_1.default.basename(dirPath))
            : dirPath;
...
const ensureRequiredDirectories = () => {
    const requiredDirs = [
        ...
        path_1.default.join(process.cwd(), 'knowledge-base/images'),
        path_1.default.join(process.cwd(), 'knowledge-base/json'),
        path_1.default.join(process.cwd(), 'knowledge-base/data'),
...
async function cleanupTempDirectories() {
    ...
    // 一時ファイル配置用ディレクトリ
    const publicImagesDir = path_1.default.join(rootDir, 'public/images');
    const publicUploadsDir = path_1.default.join(rootDir, 'public/uploads');
        ...
        try {
            ...
            for (const file of files) {
                ...
                if (stat.isDirectory()) {
                    ...
                    // ファイルの場合は検証して削除
                    await verifyAndCleanupFile(filePath, path_1.default.basename(dirPath));
                }
            }
...
async function calculateImageHash(filePath) {
    try {
        ...
        const hash = crypto_1.default.createHash('md5').update(fileContent).digest('hex');
        return hash;
    }
    catch (error) {
...
async function detectAndRemoveDuplicateImages() {
    const knowledgeImagesDir = path_1.default.join(process.cwd(), 'knowledge-base/images');
    let removedCount = 0;
    let errorCount = 0;
    ...
    try {
        ...
        for (const entry of Array.from(prefixGroups.entries())) {
            ...
            if (files.length > 1) {
                ...
                for (const file of files) {
                    const filePath = path_1.default.join(knowledgeImagesDir, file);
...
async function cleanupRedundantFiles() {
    ...
    try {
        ...
        const knowledgeImages = fs_1.default.readdirSync(knowledgeImagesDir);
        ...
        for (const dir of uploadsDirs) {
            ...
            const uploadedFiles = fs_1.default.readdirSync(dir);
            ...
            for (const file of uploadedFiles) {
                ...
                if (knowledgeImages.includes(file)) {
                    try {
                        ...
                        console.log(`重複ファイルを削除しました: ${path_1.default.join(dir, file)}`);
                        removedCount++;
                    }
                    catch (error) {
                        console.error(`ファイル削除エラー: ${path_1.default.join(dir, file)}`, error);
                        errorCount++;
                    }
                }
            }
...
async function verifyAndCleanupFile(filePath, subDir) {
    try {
        ...
        const baseNameWithoutExt = path_1.default.basename(fileName, fileExt);
        ...
        const kbTargetPath = path_1.default.join(kbTargetDir, fileName);
    ...
    try {
        ...
        for (const file of files) {
            ...
            if (stat.isDirectory()) {
                ...
                const relPath = path_1.default.relative(path_1.default.join(__dirname, '../../uploads'), dirPath);
                const topDir = relPath.split(path_1.default.sep)[0];
...
const knowledgeBaseDir = process.env.KNOWLEDGE_BASE_PATH || path_1.default.join(process.cwd(), 'knowledge-base');
const knowledgeBaseDataDir = path_1.default.join(knowledgeBaseDir, 'data');
const knowledgeBaseImagesDir = path_1.default.join(knowledgeBaseDir, 'images');
...
// 知識ベース一時ディレクトリのパス
const knowledgeBaseTempDir = path_1.default.join(knowledgeBaseDir, 'temp');
// ディレクトリが存在することを確認
...
router.post('/clear-cache', async (_req, res) => {
    try {
        ...
        // 知識ベースJSONディレクトリの再検証
        const jsonDir = path_1.default.join(process.cwd(), 'knowledge-base/json');
        if (fs_1.default.existsSync(jsonDir)) {
            try {
                // 実際のファイル一覧を取得
                const files = fs_1.default.readdirSync(jsonDir);
        ...
        const indexJsonPath = path_1.default.join(process.cwd(), 'knowledge-base/index.json');
        try {
            // 実際のファイルリストを取得
            const jsonFiles = fs_1.default.existsSync(jsonDir)
                ? fs_1.default.readdirSync(jsonDir)
                : [];
            ...
            for (const file of validFiles) {
                try {
                    const content = fs_1.default.readFileSync(path_1.default.join(jsonDir, file), 'utf8');
...
router.get('/list-json-files', (_req, res) => {
    try {
        ...
        const jsonDirs = [
            path_1.default.join(__dirname, '../../knowledge-base/json'), // メインの場所
        ];
        let allJsonFiles = [];
        ...
        for (const jsonDir of jsonDirs) {
            if (fs_1.default.existsSync(jsonDir)) {
                ...
                const allFiles = fs_1.default.readdirSync(jsonDir);
...
router.post('/init-image-search-data', async (_req, res) => {
    try {
        ...
        const existingDataPath = path_1.default.join(knowledgeBaseDataDir, 'image_search_data.json');
        let existingData = [];
        if (fs_1.default.existsSync(existingDataPath)) {
            try {
                const existingContent = fs_1.default.readFileSync(existingDataPath, 'utf-8');
        ...
        if (fs_1.default.existsSync(jsonDir)) {
            ...
            for (const jsonFile of jsonFiles) {
                const jsonPath = path_1.default.join(jsonDir, jsonFile);
                try {
                        ...
                        metadata.slides.forEach((slide, index) => {
                            if (slide['画像テキスト'] &&
                                ...
                                if (imageText && imageText['画像パス']) {
                                    const fileName = path_1.default.basename(imageText['画像パス']);
                    ...
                    if (metadata.embeddedImages &&
                        ...
                        metadata.embeddedImages.forEach((img, index) => {
                            if (img['抽出パス']) {
                                const filename = path_1.default.basename(img['抽出パス']);
                                const imagePath = `/knowledge-base/images/${filename}`;
                                // 実際にファイルが存在する場合のみ追加
                                if (existingImageFiles.includes(imagePath)) {
                                    // 元のファイル名から詳細情報を抽出
...
router.post('/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        const fileBaseName = path_1.default.basename(file.path);
        const filesDir = path_1.default.dirname(file.path);
        const processingType = req.body.processingType || 'document';
            ...
            try {
                ...
                if (fileExt !== '.png') {
                    try {
                        ...
                        // PNGファイルパスを生成
                        pngFilePath = path_1.default.join(publicImagesDir, `${path_1.default.basename(filePath, fileExt)}.png`);
                        console.log(`${fileExt}形式からPNG形式に変換: ${pngFilePath}`);
                        if (fileExt === '.svg') {
                            // SVGの場合は特別な処理
                            const svgContent = fs_1.default.readFileSync(origFilePath, 'utf8');
                            const svgBuffer = Buffer.from(svgContent);
                            await (0, sharp_1.default)(svgBuffer).png().toFile(pngFilePath);
                        }
                        else {
                            // その他の画像形式はそのままsharpで変換
                            await (0, sharp_1.default)(origFilePath).png().toFile(pngFilePath);
                        }
                ...
                const knowledgeBaseDataDir = path_1.default.join(__dirname, '../../knowledge-base/data');
                if (!fs_1.default.existsSync(knowledgeBaseDataDir)) {
                    fs_1.default.mkdirSync(knowledgeBaseDataDir, { recursive: true });
                }
                // データの保存先は knowledge-base/data のみに一元化
                const imageSearchDataPath = path_1.default.join(knowledgeBaseDataDir, 'image_search_data.json');
                // 画像検索データの初期化
                let imageSearchData = [];
                if (fs_1.default.existsSync(imageSearchDataPath)) {
                    try {
                        const jsonContent = fs_1.default.readFileSync(imageSearchDataPath, 'utf8');
                ...
                const newImageItem = {
                    id: fileId,
                    file: `/knowledge-base/images/${path_1.default.basename(updatedFilePath || filePath)}`,
                ...
                return res.json({
                    ...
                    file: {
                        ...
                        path: `/knowledge-base/images/${path_1.default.basename(updatedFilePath || filePath)}`,
        ...
        try {
            ...
            const extractedDataPath = path_1.default.join(knowledgeBaseDataDir, 'extracted_data.json');
            ...
            // 既存データの読み込み
            const extractedData = JSON.parse(fs_1.default.readFileSync(extractedDataPath, 'utf-8'));
            // 車両データキーが存在するか確認
            ...
            // 3. メタデータファイルパス生成
            const metadataFilePath = path_1.default.join(jsonDir, metadataFileName);
            ...
            // 後方互換性のために元の場所にも保存
            fs_1.default.writeFileSync(`${filePath}_metadata.json`, JSON.stringify(metadataContent, null, 2));
            // 車両データに追加
            ...
            // ナレッジベースへの追加を試みる
            try {
                await (0, knowledge_base_js_1.addDocumentToKnowledgeBase)({
...
router.post('/sync-knowledge-base', async (_req, res) => {
    try {
        ...
        const knowledgeBaseDirs = {
            images: path_1.default.join(__dirname, '../../knowledge-base/images'),
            json: path_1.default.join(__dirname, '../../knowledge-base/json'),
            data: path_1.default.join(__dirname, '../../knowledge-base/data'),
        };
        // ディレクトリが存在することだけ確認
...
router.post('/sync-directories', async (_req, res) => {
    try {
        ...
        const rootDir = path_1.default.join(__dirname, '../../');
        const knowledgeBaseImagesDir = path_1.default.join(rootDir, 'knowledge-base/images');
        const tempImageDirs = [
            path_1.default.join(rootDir, 'uploads/images'),
            path_1.default.join(rootDir, 'public/uploads/images'),
            path_1.default.join(rootDir, 'public/images'),
        ...
        for (const sourceDir of tempImageDirs) {
            ...
            for (const file of files) {
                ...
                const targetPath = path_1.default.join(knowledgeBaseImagesDir, file);
        ...
        const kbFiles = fs_1.default.readdirSync(knowledgeBaseImagesDir);
        for (const file of kbFiles) {
            const sourcePath = path_1.default.join(knowledgeBaseImagesDir, file);
...
router.get('/knowledge-base-files', async (_req, res) => {
    try {
        ...
        for (const [dirType, dir] of Object.entries(knowledgeBaseDirs)) {
            if (fs_1.default.existsSync(dir)) {
                files[dirType] = fs_1.default.readdirSync(dir).filter(file => {
...
async function cleanupOrphanedJsonFiles() {
    ...
    try {
        ...
        // 新しいドキュメント構造も考慮
        const documentsDir = path_1.default.join(knowledgeBaseDir, 'documents');
        if (fs_1.default.existsSync(documentsDir)) {
            const moreDocs = fs_1.default
...
exports.techSupportRouter = router;

...
function cleanupTempDirectory(dirPath) {
  ...
  try {
    ...
    console.error(
      `一時ディレクトリのクリーンアップに失敗しました: ${dirPath}`,
      error
    );
  }
}
...
const fileFilter = (_req: any, file: any, cb) => {
  ...
  if (allowedExtensions.includes(ext)) {
    ...
    cb(
      new Error(
        'サポートされていないファイル形式です。PowerPoint (.pptx, .ppt)、Excel (.xlsx, .xls)、PDF (.pdf)、または JSON (.json) ファイルのみアップロードできます。'
      )
    );
  }
};
  ...
  try {
    ...
    if (fileExtension.toLowerCase() === '.pptx') {
      ...
      for (let i = 0; i < slideFiles.length; i++) {
        ...
        if (fs.existsSync(mediaDir)) {
          ...
          for (const mediaFile of mediaFiles) {
            ...
            const imageText: any =
              texts.length > 0 ? texts[0] : '画像の説明がありません';
            imageTexts.push({
              画像パス: relativePath,
              テキスト: imageText,
            });
      ...
      try {
        ...
        if (!jsonData) {
          throw new Error(
            'JSONファイルの解析に失敗しました。有効なJSONファイルを確認してください。'
          );
        }
        ...
        if (isTroubleshootingFormat) {
          console.log(
            'トラブルシューティング形式のJSONを検出しました。steps配列があります。'
          );
          ...
          const convertedData = {
            metadata: {
              ...
              説明:
                ...
                'トラブルシューティングフローから生成された応急処置フローです',
              原形式: 'troubleshooting',
            },
        ...
        if (isTroubleshootingFormat) {
          ...
          console.log(
            `トラブルシューティング形式のJSONを保存しました: ${tsFilePath}`
          );
        }
        // 画像パスの修正（必要に応じて）
        ...
        // メタデータの更新
        jsonData.metadata.作成日 =
          jsonData.metadata.作成日 || new Date().toISOString();
        jsonData.metadata.修正日 = new Date().toISOString();
        // 説明を更新し、「応急復旧」を「応急処置」に統一
        if (
          jsonData.metadata.説明 &&
          jsonData.metadata.説明.includes('応急復旧')
        ) {
          jsonData.metadata.説明 = jsonData.metadata.説明.replace(
            /応急復旧/g,
            '応急処置'
          );
        }
        ...
        throw new Error(
          `JSONファイルの処理に失敗しました: ${error instanceof Error ? error.message : '不明なエラー'}`
        );
      }
    } else {
...
router.post('/process', upload.single('file'), async (req, res) => {
  try {
    ...
    console.log(
      `ファイルはknowledge-baseディレクトリに直接処理されました: ${result.filePath}`
    );
    ...
    const guides: any = files.map(file => {
      try {
        ...
        return {
          ...
          fileName: file,
          title:
            data.metadata?.タイトル || path.basename(file, '_metadata.json'),
  ...
  try {
    ...
    if (id.startsWith('ts_')) {
      ...
      if (!fs.existsSync(filePath)) {
        return res
          .status(404)
          .json({ error: 'トラブルシューティングファイルが見つかりません' });
      }
      const content: any = fs.readFileSync(filePath, 'utf8');
    ...
    if (id.startsWith('ts_')) {
      ...
      if (data.slides && Array.isArray(data.slides)) {
        updatedTsData.steps = data.slides.map((slide, index) => ({
          ...
          description: Array.isArray(slide.本文)
            ? slide.本文.join('\n')
            : slide.本文 || '',
          message: Array.isArray(slide.本文)
            ? slide.本文.join('\n')
            : slide.本文 || '',
  ...
  try {
    ...
    if (id.startsWith('ts_')) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        fs.unlinkSync(filePath);
        console.log(
          `トラブルシューティングファイルを削除しました: ${filePath}`
        );
      }
      // 対応するメタデータファイルも削除
      const guideFileName = `ts_${tsId}_metadata.json`;
...
router.post('/send-to-chat/:guideId/:chatId', async (_req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: '応急処置フローのチャットへの送信に失敗しました' });
  }
});

...
router.post('/clear-cache', async (req, res) => {
    try {
        ...
        try {
            ...
            // 有効なメタデータファイルのみを追加
            const validFiles = jsonFiles.filter(file => file.endsWith('_metadata.json') &&
...
router.post('/init-image-search-data', async (req, res) => {
    try {
        ...
        let newData = [];
        if (fs.existsSync(jsonDir)) {
            const jsonFiles = fs.readdirSync(jsonDir).filter(file => file.endsWith('_metadata.json') && !file.includes('guide_'));
...
async function cleanupOrphanedJsonFiles() {
    ...
    try {
        ...
        // メタデータJSONファイル一覧を取得
        const allFiles = fs.readdirSync(jsonDir);
        const metadataFiles = allFiles.filter(file => file.endsWith('_metadata.json') &&
            !blacklistFiles.includes(file));

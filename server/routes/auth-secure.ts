...
function validatePassword(password: string): { valid: boolean; message?: string } {
  ...
  if (!hasUpperCase) {
    return { valid: false, message: 'パスワードには大文字が含まれている必要があります' };
  }
  
  if (!hasLowerCase) {
    return { valid: false, message: 'パスワードには小文字が含まれている必要があります' };
  }
  
  if (!hasNumbers) {
    return { valid: false, message: 'パスワードには数字が含まれている必要があります' };
  }
  
  if (!hasSpecialChar) {
    return { valid: false, message: 'パスワードには特殊文字が含まれている必要があります' };
  }
...
function validateUsername(username: string): { valid: boolean; message?: string } {
  ...
  if (username.length > 30) {
    return { valid: false, message: 'ユーザー名は30文字以下である必要があります' };
  }
  
  if (!/^[a-zA-Z0-9_-]+$/.test(username)) {
    return { valid: false, message: 'ユーザー名は英数字、アンダースコア、ハイフンのみ使用できます' };
  }

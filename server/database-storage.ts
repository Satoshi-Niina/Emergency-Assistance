...
export class DatabaseStorage {
  ...
  getChat = async (id: string): Promise<any> => {
    const chat = (
      await db.select().from(schema.chats).where(eq(schema.chats.id, id))
    )[0];
    return chat;
  };
  ...
  createMessage = async (message: any): Promise<any> => {
    ...
    const newMessage = (
      await db.insert(schema.messages).values(insertData).returning()
    )[0];
    ...
    while (attempt < 3) {
      try {
        ...
        if (remainingMessages.length === 0) {
          console.log(
            `チャットメッセージクリア完了: chatId=${chatId}, 削除メディア=${deletedMediaCount}, 削除メッセージ=${deletedMessageCount}`
          );
          break;
        }
        console.warn(
          `試行 ${attempt + 1} 後に ${remainingMessages.length} 件のメッセージが残っています`
        );
        if (attempt === 2) {
  ...
  getMedia = async (id: string): Promise<any> => {
    const mediaItem = (
      await db.select().from(schema.media).where(eq(schema.media.id, id))
    )[0];
    return mediaItem;
  };
  ...
  createMedia = async (mediaItem: any): Promise<any> => {
    const newMedia = (
      await db.insert(schema.media).values(mediaItem).returning()
    )[0];
    return newMedia;
  };
  // Keyword methods
  ...
  createKeyword = async (keyword: any): Promise<any> => {
    const newKeyword = (
      await db.insert(schema.keywords).values(keyword).returning()
    )[0];
    return newKeyword;
  };
  searchDocumentsByKeyword = async (keyword: string): Promise<any[]> => {
    ...
    const documentIds = Array.from(
      new Set(matchingKeywords.map((k: any) => k.documentId))
    );
  ...
  // チャットエクスポート関連のメソッド
  getMessagesForChatAfterTimestamp = async (
    chatId: string,
    timestamp: number
  ): Promise<any[]> => {

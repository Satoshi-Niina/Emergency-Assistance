...
export function generateInteractiveResponse(
  ...
  const detailRequested =
    !!userResponse && /詳細|もっと詳しく|くわしく|詳しく教/.test(userResponse);

  // 安全確認が最優先
  if (
    state.collectedInfo.urgency === 'critical' &&
    ...
    return decorateWithDetail(
      {
        ...
        nextQuestion:
          '現場は安全ですか？ 人の立入りなし / 機械は完全停止 / 火気・漏れなし を確認してください。',
        priority: 'safety',
        requiresInput: true,
        phase: 'investigation',
      },
...
function decorateWithDetail(
  base: InteractiveResponse,
  state: DiagnosisState,
  ...
  if (detailRequested) {
    ...
    detailParts.push(
      `緊急度: ${state.collectedInfo.urgency}  信頼度: ${Math.round(state.confidence * 100)}%`
    );
...
function toFormal(text: string): string {
  ...
  // カジュアル語尾の簡易正規化
  t = t.replace(
    /(しましょう|していきますね|していきます|進めていきますね|進めてくださいね|進めてください)/g,
    'します'
  );
  t = t.replace(
    /(見ていきましょう|確認しましょう|一緒に見ていきましょう)/g,
    '確認します'
  );
  t = t.replace(
    /(教えてくださいね|教えてくださいね。|教えてください。?|教えてください)/g,
    '提示してください'
  );
...
function generateInitialResponse(state: DiagnosisState): InteractiveResponse {
  ...
  return {
    message:
      `${v(state, 'initial.opener', openers)} ${v(state, 'initial.ask', ask)}`.trim(),
    details:
      '例: 始動不可 / 異音 / 作業装置不作動 / 警告灯点灯 など。複数は重要度順に。',
...
function generateInvestigationResponse(
  state: DiagnosisState,
  userResponse?: string
): InteractiveResponse {
...
function generateActionResponse(state: DiagnosisState): InteractiveResponse {
  ...
  const primaryCause = suspectedCauses[0];
  const stepByStepActions = generateStepByStepActions(
    primaryCause,
    collectedInfo.vehicleType
  );
  // 1ステップずつ案内
  let stepIdx =
    state.nextActions && state.nextActions.length
...
function generateVerificationResponse(
  state: DiagnosisState
): InteractiveResponse {
  ...
  return {
    ...
    details:
      '例: 完全に解決 / 部分的に改善(どこが残存) / 変化なし / 悪化(追加症状)',
...
function generateCompletedResponse(state: DiagnosisState): InteractiveResponse {
  return {
    ...
    details:
      'ログ保存推奨: 1) 点検周期の見直し 2) 兆候の早期共有 3) 恒久対策が必要なら計画化しましょう。',
    priority: 'info',
    requiresInput: false,
    phase: 'completed',
  };
...
function assessUrgency(
  info: DiagnosisState['collectedInfo']
...
function generateSuspectedCauses(
  info: DiagnosisState['collectedInfo']
): string[] {
  const causes = [];
...
function generateStepByStepActions(
  cause: string,
  vehicleType: string | null
): string[] {

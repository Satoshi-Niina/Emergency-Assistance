 ...
 */
export async function searchKnowledgeBase(query, ragSettings) {
    // インメモリで単純な検索を実装
    try {
 ...
 */
export async function generateSystemPromptWithKnowledge(query, ragSettings) {
 ...
 * @returns 処理結果
 */
export function addDocumentToKnowledgeBase(fileInfo, content) {
    try {
        // ファイル名から拡張子を除いた部分を取得
 ...
 */
export function determineKnowledgeType(filename, content) {
 ...
 */
export function saveKnowledgeData(filename, content, metadata) {
    try {
        const timestamp = Date.now();
 ...
 * ナレッジデータの一覧を取得
 */
export function listKnowledgeData(type) {
    try {
        const index = loadKnowledgeBaseIndex();
        if (!index.knowledge) {
            return {
 ...
 * @returns マージされたコンテンツ
 */
export function mergeDocumentContent(contents) {
    // 単純に改行で区切ってマージする
 ...
 * @returns インデックスデータ
 */
export function loadKnowledgeBaseIndex() {
    try {
 ...
 * @returns 削除結果
 */
export function removeDocumentFromKnowledgeBase(documentId) {
    try {

...
function updateDiagnosisState(currentState, userResponse) {
    ...
    if (currentState.phase === 'action' && userResponse) {
        ...
        const actions = generateStepByStepActions(currentState.suspectedCauses[0], currentState.collectedInfo.vehicleType);
...
function generateInteractiveResponse(state, userResponse) {
    ...
    if (state.collectedInfo.urgency === 'critical' &&
        !state.collectedInfo.safetyStatus) {
    ...
    switch (state.phase) {
        case 'initial':
            base = generateInitialResponse(state);
            break;
        case 'investigation':
            base = generateInvestigationResponse(state, userResponse);
            break;
        case 'diagnosis':
            base = generateDiagnosisResponse(state);
            break;
        case 'action':
            base = generateActionResponse(state);
            break;
        case 'verification':
            base = generateVerificationResponse(state);
            break;
        default:
            base = generateCompletedResponse(state);
            break;
    }
    // 重複質問検知 & 再表現
    if (base.nextQuestion &&
        state.lastQuestion &&
        normalizeQ(base.nextQuestion) === normalizeQ(state.lastQuestion)) {
...
function decorateWithDetail(base, state, detailRequested) {
...
function enrichOptions(options, detailRequested) {
    const base = options ? [...options] : [];
    if (!detailRequested && !base.includes('詳細'))
        base.push('詳細');
    if (!base.includes('別の可能性'))
        base.push('別の可能性');
    if (!base.includes('安全再確認'))
        base.push('安全再確認');
    return base;
}
// ---------- フォーマル変換 & 重複処理ユーティリティ ----------
function attachFormalVariants(resp) {
    const formalMessage = toFormal(resp.message);
    const formalNext = resp.nextQuestion
        ? toFormal(resp.nextQuestion)
        : undefined;
...
function rephraseQuestion(original) {
    // シンプルな再表現: 末尾に具体化依頼を追加
    if (/具体|詳/.test(original))
        return original + '（前回答との差分を明確にしてください）';
    return (original.replace(/？?$/, '') +
        'を、より具体的な名詞または数値で示してください。');
}
function generateInitialResponse(state) {
    const openers = [
        '🔧 まず状況の共有から始めましょうね。',
...
function generateInvestigationResponse(state, userResponse) {
...
function generateDiagnosisResponse(state) {
    ...
    if (confidence >= 0.7 && suspectedCauses.length > 0) {
        ...
        return {
            ...
            details: suspectedCauses.length
                ? suspectedCauses.map((c, i) => `${i + 1}. ${c}`).join('\n')
                : 'まだ十分な特徴がありません。',
...
function generateActionResponse(state) {
    ...
    // 1ステップずつ案内
    let stepIdx = state.nextActions && state.nextActions.length
        ? state.nextActions.length
        : 0;
    if (stepIdx >= stepByStepActions.length)
        stepIdx = stepByStepActions.length - 1;
...
function generateVerificationResponse(state) {
...
function generateCompletedResponse(state) {
    return {
...
function detectVehicleType(response) {
...
function detectSafetyStatus(response) {
    if (/安全|大丈夫|停止/.test(response))
        return 'safe';
    if (/危険|不安|動いている/.test(response))
        return 'unsafe';
    return null;
}
function assessUrgency(info) {
    const criticalSymptoms = ['異常発熱・煙', '異常振動'];
    const urgentSymptoms = ['エンジン始動不良', '動作不良'];
    if (info.symptoms.some(s => criticalSymptoms.includes(s)))
        return 'critical';
    if (info.symptoms.some(s => urgentSymptoms.includes(s)))
        return 'high';
    if (info.symptoms.length > 1)
        return 'medium';
    return 'low';
}
function generateSuspectedCauses(info) {
    const causes = [];
...
function determineNextPhase(state) {
    ...
    if (!safetyStatus && state.collectedInfo.urgency !== 'low')
        return 'investigation';
    if (symptoms.length === 0)
        return 'initial';
    if (!vehicleType)
        return 'investigation';
    if (state.suspectedCauses.length === 0)
        return 'investigation';
...
function calculateDiagnosisConfidence(state) {
    let confidence = 0.2;
    if (state.collectedInfo.symptoms.length > 0)
        confidence += 0.3;
    if (state.collectedInfo.vehicleType)
        confidence += 0.2;
    if (state.collectedInfo.safetyStatus)
        confidence += 0.1;
    if (state.suspectedCauses.length > 0)
        confidence += 0.2;
    return Math.min(confidence, 1.0);
}
function generateSpecificQuestions(symptoms, vehicleType) {
    const questions = [];
...
function generateInitialActions(cause) {
    const actionMap = {
        バッテリー不良: [
            'バッテリー電圧確認',
            '端子清掃',
...
function generateStepByStepActions(cause, vehicleType) {
    // 車両タイプと原因に応じた詳細なステップを生成
    const baseActions = generateInitialActions(cause);
    return baseActions.map((action, index) => `${action}（${vehicleType || '保守用車'}専用手順に従って実施）`);
}
// ---------- 可変表現ユーティリティ ----------
function v(state, group, arr) {
    if (!arr.length)
        return '';
    if (arr.length === 1)
        return arr[0];
    ...
    const candidates = arr.map((text, idx) => ({
        text,
        key: `${group}:${idx}`,
        idx,
    }));
    ...
    if (!state.phraseHistory)
        state.phraseHistory = [];
    state.phraseHistory.push(chosen.key);
    if (state.phraseHistory.length > 100)
        state.phraseHistory.splice(0, state.phraseHistory.length - 100);
    return chosen.text;
}

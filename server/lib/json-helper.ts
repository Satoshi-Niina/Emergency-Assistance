...
export function cleanJsonResponse(response: string): string {
  ...
  try {
    ...
    if (errorPosition >= 0) {
      ...
      console.log(
        `問題のある箇所: "${cleanedResponse.substring(contextStart, errorPosition)}<<<HERE>>>${cleanedResponse.substring(errorPosition, contextEnd)}"`
      );
...
function repairJsonAtPosition(json: string, errorPosition: number): string {
  ...
  if (
    ...
    beforeError.lastIndexOf('[') > beforeError.lastIndexOf('{')
  ) {
  ...
  if (balanceInfo.bracketCount > 0 && balanceInfo.lastArrayDelimiter > 0) {
    ...
    const validPart = beforeError.substring(
      0,
      balanceInfo.lastArrayDelimiter + 1
    );
    const remainingBrackets = balanceInfo.bracketCount;
    // 閉じ括弧を追加
    const fixedJson =
      validPart +
      ']'.repeat(remainingBrackets) +

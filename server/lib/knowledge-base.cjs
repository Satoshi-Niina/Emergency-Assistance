...
exports.KnowledgeType = exports.INDEX_FILE = void 0;
...
exports.determineKnowledgeType = determineKnowledgeType;
exports.saveKnowledgeData = saveKnowledgeData;
exports.listKnowledgeData = listKnowledgeData;
exports.getKnowledgeData = getKnowledgeData;
exports.deleteKnowledgeData = deleteKnowledgeData;
...
// CommonJS環境では__dirname は自動的に定義されているため、追加の定義は不要
// 知識ベースディレクトリのパス解決
...
// 知識ベースインデックスファイル
exports.INDEX_FILE = path.join(KNOWLEDGE_BASE_DIR, 'index.json');
// ナレッジデータの種類
var KnowledgeType;
(function (KnowledgeType) {
    ...
    KnowledgeType["QA"] = "qa";
    KnowledgeType["JSON"] = "json";
    KnowledgeType["PPT"] = "ppt";
 ...
 * @returns 類似度スコア（0-1）
 */
function calculateEnhancedSimilarity(query, text, metadata) {
    // 基本の類似度計算
    ...
    const matchedTechnicalTerms = queryWords.filter(word => technicalTerms.some(term => term.includes(word) || word.includes(term)));
    ...
    // 最終スコアの計算
    const finalScore = Math.min(1.0, baseSimilarity + importanceBonus + technicalBonus + exactMatchBonus) * lengthNormalization;
    return finalScore;
}
/**
 * 知識ベースから検索する関数
 ...
 */
async function searchKnowledgeBase(query, ragSettings) {
    // インメモリで単純な検索を実装
    try {
 ...
 */
async function generateSystemPromptWithKnowledge(query, ragSettings) {
    ...
    if (relevantChunks.length > 0) {
        ...
        // 緊急度・重要度別にチャンクを分類
        const urgentChunks = sortedChunks.filter(chunk => chunk.metadata.isImportant &&
            (chunk.text.includes('緊急') ||
                chunk.text.includes('危険') ||
                chunk.text.includes('注意')));
        const importantChunks = sortedChunks.filter(chunk => chunk.metadata.isImportant && !urgentChunks.includes(chunk));
 ...
 */
function determineKnowledgeType(filename, content) {
 ...
 */
function saveKnowledgeData(filename, content, metadata) {
    try {
        const timestamp = Date.now();
        ...
        // インデックスを保存
        fs.writeFileSync(exports.INDEX_FILE, JSON.stringify(index, null, 2));
 ...
 * ナレッジデータの一覧を取得
 */
function listKnowledgeData(type) {
    try {
        const index = loadKnowledgeBaseIndex();
        if (!index.knowledge) {
            return {
 ...
 * ナレッジデータを削除
 */
function deleteKnowledgeData(id) {
    try {
        const index = loadKnowledgeBaseIndex();
        if (!index.knowledge) {
            return {
...
function loadKnowledgeBaseIndex() {
    try {
        if (!fs.existsSync(exports.INDEX_FILE)) {
        ...
        const indexContent = fs.readFileSync(exports.INDEX_FILE, 'utf-8');
    ...
    try {
        ...
        const documents = textFiles.map(file => {
            try {
                ...
                const timestamp = parseInt(nameParts[nameParts.length - 1], 10) ||
                    stats.mtime.getTime();
                return {
                    id: file.replace('.txt', ''),
                    filename: file,

...
// 症状のサンプルデータ
const symptoms = [
    { id: 'engine-stop', description: 'エンジン停止', keywords: ['エンジン停止', 'エンジンが止まる', 'エンジン切れ', 'エンジンが起動しない', 'エンジンが始動しない'] },
    { id: 'engine-noise', description: '異音', keywords: ['異音', '変な音', '音がする', 'ノッキング', 'バルブ音', 'ピストン音'] },
    { id: 'engine-overheat', description: 'オーバーヒート', keywords: ['オーバーヒート', '水温上昇', 'エンジンが熱い', '冷却水不足'] },
    { id: 'brake-failure', description: 'ブレーキ不良', keywords: ['ブレーキ不良', 'ブレーキが効かない', 'ブレーキ故障', 'ブレーキが解放しない', 'ブレーキが利かない'] },
    { id: 'brake-noise', description: 'ブレーキ異音', keywords: ['ブレーキ異音', 'ブレーキから音', 'ブレーキ鳴き'] },
    { id: 'hydraulic-leak', description: '油圧漏れ', keywords: ['油圧漏れ', 'オイル漏れ', '漏油', '油が漏れる', 'オイルが漏れる'] },
    { id: 'hydraulic-pressure', description: '油圧不足', keywords: ['油圧不足', '油圧が低い', '油圧警告', '油圧低下'] },
    { id: 'electrical-failure', description: '電気系統故障', keywords: ['電気系統', '電装品', '電気不良', '電気故障', '配線不良'] },
    { id: 'electrical-short', description: '電気ショート', keywords: ['電気ショート', 'ショート', '配線ショート', '電気火花'] },
    { id: 'transmission-failure', description: '変速機故障', keywords: ['変速機故障', 'ギアが入らない', '変速不良', 'トランスミッション故障'] },
    { id: 'transmission-noise', description: '変速機異音', keywords: ['変速機異音', 'ギア音', '変速音'] },
    { id: 'air-pressure', description: '空気圧不足', keywords: ['空気圧不足', 'エアーがたまらない', '空気圧警告', 'エアー圧低下'] },
    { id: 'battery-dead', description: 'バッテリー上がり', keywords: ['バッテリー上がり', 'バッテリー切れ', 'バッテリー不足', '電圧不足'] },
    { id: 'warning-light', description: '警告灯点灯', keywords: ['警告灯', '警告ランプ', 'アラーム', '警告音'] },
    { id: 'operation-failure', description: '操作不良', keywords: ['操作不良', '操作できない', '動作しない', '反応しない'] },
    { id: 'vibration', description: '振動', keywords: ['振動', '揺れ', 'がたつき', '振動する'] },
    { id: 'smoke', description: '煙', keywords: ['煙', '白煙', '黒煙', '煙が出る'] },
    { id: 'fuel-problem', description: '燃料問題', keywords: ['燃料不足', '燃料切れ', 'ガソリン不足', '軽油不足'] },
    { id: 'cooling-problem', description: '冷却問題', keywords: ['冷却水不足', '冷却水漏れ', 'ラジエーター', '冷却不良'] },
];

// コンポーネントのサンプルデータ
const components = [
    { id: 'engine', name: 'エンジン', keywords: ['エンジン', 'engine', 'モーター', '原動機'] },
    { id: 'brake', name: 'ブレーキ', keywords: ['ブレーキ', 'brake', '制動装置', '制動器'] },
    { id: 'hydraulic', name: '油圧系統', keywords: ['油圧', 'hydraulic', 'オイル', '油', '油圧系統', '油圧装置'] },
    { id: 'electrical', name: '電気系統', keywords: ['電気', 'electrical', '電装', '配線', '電気系統', '電装品'] },
    { id: 'transmission', name: '変速機', keywords: ['変速機', 'transmission', 'ギア', 'トランスミッション', '変速装置'] },
    { id: 'air-system', name: '空気系統', keywords: ['空気', 'エアー', 'air', '空気系統', 'エアー系統'] },
    { id: 'battery', name: 'バッテリー', keywords: ['バッテリー', 'battery', '蓄電池', '電源'] },
    { id: 'cooling', name: '冷却系統', keywords: ['冷却', 'cooling', 'ラジエーター', '冷却水', '冷却系統'] },
    { id: 'fuel', name: '燃料系統', keywords: ['燃料', 'fuel', 'ガソリン', '軽油', '燃料系統'] },
    { id: 'pump', name: 'ポンプ', keywords: ['ポンプ', 'pump', '油圧ポンプ', '燃料ポンプ'] },
    { id: 'motor', name: 'モーター', keywords: ['モーター', 'motor', '電動機', '駆動モーター'] },
    { id: 'sensor', name: 'センサー', keywords: ['センサー', 'sensor', '検出器', '感知器'] },
    { id: 'valve', name: 'バルブ', keywords: ['バルブ', 'valve', '弁', '制御弁'] },
    { id: 'filter', name: 'フィルター', keywords: ['フィルター', 'filter', '濾過器', 'エアフィルター', 'オイルフィルター'] },
    { id: 'hose', name: 'ホース', keywords: ['ホース', 'hose', '配管', '油圧ホース', 'エアーホース'] },
];
...
async function generateJapaneseTitle(userMessages: string, allMessages?: string): Promise<string> {
            ...
            if (allTextOnly) {
                ...
                if (extractedComponents.length > 0 && extractedSymptoms.length > 0) {
                    ...
                    if (symptom.includes('故障') || symptom.includes('不良')) {
                        ...
                        return `${component}警告`;
                    } else if (symptom.includes('動作不良') || symptom.includes('動作しない')) {
                        return `${component}動作不良`;
                    } else if (symptom.includes('オーバーヒート') || symptom.includes('熱い')) {
    ...
    if (extractedComponents.length > 0 && extractedSymptoms.length > 0) {
        ...
        if (symptom.includes('故障') || symptom.includes('不良')) {
            title = `${component}故障`;
        } else if (symptom.includes('停止') || symptom.includes('止ま') || symptom.includes('起動しない') || symptom.includes('始動しない')) {
            title = `${component}停止`;
        } else if (symptom.includes('急に停止') || symptom.includes('突然停止')) {
            title = `${component}急に停止`;
        } else if (symptom.includes('異音') || symptom.includes('音') || symptom.includes('ノッキング') || symptom.includes('バルブ音')) {
            title = `${component}異音`;
        } else if (symptom.includes('漏れ') || symptom.includes('漏油') || symptom.includes('油が漏れる')) {
            title = `${component}漏れ`;
        } else if (symptom.includes('警告') || symptom.includes('アラーム') || symptom.includes('警告灯')) {
            title = `${component}警告`;
        } else if (symptom.includes('動作不良') || symptom.includes('動作しない') || symptom.includes('操作不良') || symptom.includes('反応しない')) {
            title = `${component}動作不良`;
        } else if (symptom.includes('オーバーヒート') || symptom.includes('水温上昇') || symptom.includes('熱い')) {
            title = `${component}オーバーヒート`;
        } else if (symptom.includes('不足') || symptom.includes('低下')) {
            title = `${component}不足`;
        } else if (symptom.includes('振動') || symptom.includes('揺れ') || symptom.includes('がたつき')) {
            title = `${component}振動`;
        } else if (symptom.includes('煙') || symptom.includes('白煙') || symptom.includes('黒煙')) {
            title = `${component}煙`;

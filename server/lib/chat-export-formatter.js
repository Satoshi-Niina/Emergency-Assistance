 ...
 */
async function generateJapaneseTitle(userMessages, allMessages) {
        ...
        if (allMessages) {
            ...
            if (allTextOnly) {
                ...
                if (extractedComponents.length > 0 && extractedSymptoms.length > 0) {
                    ...
                    else if (symptom.includes('動作不良') ||
                        symptom.includes('動作しない')) {
                        return `${component}動作不良`;
                    }
                    else if (symptom.includes('オーバーヒート') ||
                        symptom.includes('熱い')) {
    ...
    if (extractedComponents.length > 0 && extractedSymptoms.length > 0) {
        ...
        else if (symptom.includes('異音') ||
            symptom.includes('音') ||
            symptom.includes('ノッキング') ||
            symptom.includes('バルブ音')) {
            title = `${component}異音`;
        }
        else if (symptom.includes('漏れ') ||
            symptom.includes('漏油') ||
            symptom.includes('油が漏れる')) {
            title = `${component}漏れ`;
        }
        else if (symptom.includes('警告') ||
        ...
        else if (symptom.includes('振動') ||
            symptom.includes('揺れ') ||
            symptom.includes('がたつき')) {
            title = `${component}振動`;
        }
        else if (symptom.includes('煙') ||
            symptom.includes('白煙') ||
            symptom.includes('黒煙')) {
            title = `${component}煙`;
        }
...
export async function formatChatHistoryForHistoryUI(chat, messages, messageMedia, machineInfo) {
    ...
    const conversationHistory = messages.map((message) => {
        ...
        if (messageMedia && typeof messageMedia === 'object') {
            ...
            mediaInfo = messageMediaItems.map((media) => ({
                id: media.id,
                type: media.type,
    ...
    const historyData = {
        ...
        metadata: {
            ...
            user_messages: messages.filter((m) => !m.isAiResponse).length,
            ai_messages: messages.filter((m) => m.isAiResponse).length,
...
`;
    ...
    const conversationHistory = messages.map((message) => {
        ...
        if (messageMedia && typeof messageMedia === 'object') {
            ...
            const messageMediaItems = messageMedia.filter((media) => media.messageId === message.id);
            mediaInfo = messageMediaItems.map((media) => ({
                id: media.id,
                type: media.type,
                filename: media.filename,

...
async function applyMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(
      __dirname,
      '../../migrations/0003_fix_schema_final.sql'
    );

    // usersテーブル用のマイグレーションファイルのパス
    const usersMigrationPath = path.join(
      __dirname,
      '../../migrations/0004_add_users_table.sql'
    );

    if (!fs.existsSync(migrationPath)) {
      throw new Error(
        `マイグレーションファイルが見つかりません: ${migrationPath}`
      );
    }
    ...
    const machineTypeCount =
      await client`SELECT COUNT(*) as count FROM machine_types`;
    const historyCount =
      await client`SELECT COUNT(*) as count FROM history_items`;
